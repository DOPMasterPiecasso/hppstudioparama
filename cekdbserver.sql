-- phpMyAdmin SQL Dump
-- version 5.2.3
-- https://www.phpmyadmin.net/
--
-- Host: 127.0.0.1:3306
-- Generation Time: Jun 10, 2026 at 05:36 AM
-- Server version: 8.0.45-0ubuntu0.22.04.1
-- PHP Version: 8.1.34

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";

START TRANSACTION;

SET time_zone = "+00:00";

/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */
;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */
;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */
;
/*!40101 SET NAMES utf8mb4 */
;

--
-- Database: `hppstudioparama`
--
CREATE DATABASE IF NOT EXISTS `hppstudioparama` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;

USE `hppstudioparama`;

-- --------------------------------------------------------

--
-- Table structure for table `addons`
--

CREATE TABLE `addons` (
    `id` int NOT NULL,
    `name` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `price` bigint NOT NULL,
    `unit` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `category` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `description` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `active` tinyint DEFAULT '1',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `addons`
--

INSERT INTO
    `addons` (
        `id`,
        `name`,
        `price`,
        `unit`,
        `category`,
        `description`,
        `active`,
        `created_at`,
        `updated_at`
    )
VALUES (
        39,
        'Binding Paku/Jepang/Spiral',
        50000,
        'flat',
        'finishing',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        40,
        'Pop Up 2D',
        55000,
        'flat',
        'finishing',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        41,
        'Cover Tunnel',
        75000,
        'flat',
        'finishing',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        42,
        'Cover Klip/Cetekan',
        15000,
        'flat',
        'finishing',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        43,
        'Cover Bahan',
        55000,
        'flat',
        'finishing',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        44,
        'Ivory Paper',
        450,
        'per_hal',
        'kertas',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        45,
        'Laminasi Paper',
        600,
        'per_hal',
        'kertas',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        46,
        'Halaman Tambahan',
        3000,
        'extra_hal',
        'halaman',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        47,
        'Drone Video (1–2 mnt)',
        1500000,
        'flat_video',
        'video',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        48,
        'Docudrama Video (5–10 mnt)',
        3000000,
        'flat_video',
        'video',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        49,
        'Slide Box',
        45000,
        'flat',
        'pkg1',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        50,
        'Standart Box 1',
        150000,
        'flat',
        'pkg1',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        51,
        'Standart Box 2',
        150000,
        'flat',
        'pkg1',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        52,
        'Hard Box 3 (Akrilik)',
        125000,
        'flat',
        'pkg1',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        53,
        'Custom Box 1',
        200000,
        'flat',
        'pkg2',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        54,
        'Custom Box 2',
        165000,
        'flat',
        'pkg2',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        55,
        'Custom Box 3',
        200000,
        'flat',
        'pkg2',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        56,
        'Custom Box 4',
        200000,
        'flat',
        'pkg2',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        57,
        'Custom Box 5',
        200000,
        'flat',
        'pkg2',
        '',
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    );

-- --------------------------------------------------------

--
-- Table structure for table `addon_categories`
--

CREATE TABLE `addon_categories` (
    `id` int NOT NULL,
    `category_name` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `display_order` int DEFAULT '0',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `addon_categories`
--

INSERT INTO
    `addon_categories` (
        `id`,
        `category_name`,
        `display_order`,
        `created_at`
    )
VALUES (
        1,
        'Finishing',
        0,
        '2026-04-06 05:03:41'
    ),
    (
        2,
        'Kertas',
        0,
        '2026-04-06 05:03:41'
    ),
    (
        3,
        'Halaman Tambahan',
        0,
        '2026-04-06 05:03:41'
    ),
    (
        4,
        'Video',
        0,
        '2026-04-06 05:03:41'
    ),
    (
        5,
        'Packaging Standard',
        0,
        '2026-04-06 05:03:41'
    ),
    (
        6,
        'Custom Box',
        0,
        '2026-04-06 05:03:41'
    );

-- --------------------------------------------------------

--
-- Table structure for table `addon_items`
--

CREATE TABLE `addon_items` (
    `id` int NOT NULL,
    `category_id` int NOT NULL,
    `name` varchar(150) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `addon_type` enum(
        'flat',
        'tiered',
        'flat_video',
        'per_hal',
        'extra_hal'
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'flat',
    `description` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `flat_price` int DEFAULT NULL,
    `display_order` int DEFAULT '0',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `addon_items`
--

INSERT INTO
    `addon_items` (
        `id`,
        `category_id`,
        `name`,
        `addon_type`,
        `description`,
        `flat_price`,
        `display_order`,
        `created_at`,
        `updated_at`
    )
VALUES (
        1,
        1,
        'Hardcover',
        'tiered',
        NULL,
        NULL,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        2,
        1,
        'Softcover',
        'tiered',
        NULL,
        NULL,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        3,
        2,
        'Art Paper 260gsm',
        'tiered',
        NULL,
        NULL,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        4,
        2,
        'Glossy 230gsm',
        'tiered',
        NULL,
        NULL,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        5,
        3,
        'Halaman Tambahan',
        'extra_hal',
        NULL,
        NULL,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        6,
        4,
        'Drone Video',
        'flat_video',
        NULL,
        1500000,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        7,
        4,
        'Docudrama Video',
        'flat_video',
        NULL,
        3000000,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        8,
        5,
        'Slide Box',
        'tiered',
        NULL,
        NULL,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        9,
        6,
        'Custom Printed Box',
        'tiered',
        NULL,
        NULL,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    );

-- --------------------------------------------------------

--
-- Table structure for table `addon_tiers`
--

CREATE TABLE `addon_tiers` (
    `id` int NOT NULL,
    `addon_item_id` int NOT NULL,
    `tier_label` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `min_quantity` int NOT NULL,
    `max_quantity` int DEFAULT NULL,
    `price` int NOT NULL,
    `display_order` int DEFAULT '0',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `addon_tiers`
--

INSERT INTO
    `addon_tiers` (
        `id`,
        `addon_item_id`,
        `tier_label`,
        `min_quantity`,
        `max_quantity`,
        `price`,
        `display_order`,
        `created_at`
    )
VALUES (
        1,
        1,
        '25–75 buku',
        25,
        75,
        75000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        2,
        1,
        '76–150 buku',
        76,
        150,
        65000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        3,
        1,
        '>151 buku',
        151,
        NULL,
        55000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        4,
        2,
        '25–75 buku',
        25,
        75,
        45000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        5,
        2,
        '76–150 buku',
        76,
        150,
        35000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        6,
        2,
        '>151 buku',
        151,
        NULL,
        30000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        7,
        3,
        '25–50',
        25,
        50,
        5000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        8,
        3,
        '51–100',
        51,
        100,
        4000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        9,
        3,
        '101–150',
        101,
        150,
        3500,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        10,
        3,
        '>151',
        151,
        NULL,
        3000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        11,
        4,
        '25–50',
        25,
        50,
        3500,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        12,
        4,
        '51–100',
        51,
        100,
        3000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        13,
        4,
        '101–150',
        101,
        150,
        2500,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        14,
        4,
        '>151',
        151,
        NULL,
        2000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        15,
        5,
        '25–50 order',
        25,
        50,
        15000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        16,
        5,
        '51–100 order',
        51,
        100,
        12000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        17,
        5,
        '101–150 order',
        101,
        150,
        10000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        18,
        5,
        '>151 order',
        151,
        NULL,
        8000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        19,
        8,
        '25–50',
        25,
        50,
        25000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        20,
        8,
        '51–100',
        51,
        100,
        20000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        21,
        8,
        '101–150',
        101,
        150,
        18000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        22,
        8,
        '151–200',
        151,
        200,
        15000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        23,
        8,
        '>200',
        201,
        NULL,
        12000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        24,
        9,
        '25–50',
        25,
        50,
        45000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        25,
        9,
        '51–100',
        51,
        100,
        38000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        26,
        9,
        '101–150',
        101,
        150,
        32000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        27,
        9,
        '151–200',
        151,
        200,
        28000,
        0,
        '2026-04-06 05:03:41'
    ),
    (
        28,
        9,
        '>200',
        201,
        NULL,
        24000,
        0,
        '2026-04-06 05:03:41'
    );

-- --------------------------------------------------------

--
-- Table structure for table `alacarte_config`
--

CREATE TABLE `alacarte_config` (
    `id` int UNSIGNED NOT NULL,
    `package_code` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Kode unik paket, cth: ac-fotohalf',
    `label` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Nama tampilan paket',
    `price_min` int UNSIGNED NOT NULL DEFAULT '0' COMMENT 'Harga minimum (Rp)',
    `price_max` int UNSIGNED NOT NULL DEFAULT '0' COMMENT 'Harga maksimum (Rp). Sama dgn price_min jika harga tunggal.',
    `description` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Keterangan singkat untuk tampilan kalkulator',
    `display_order` tinyint UNSIGNED NOT NULL DEFAULT '0',
    `active` tinyint(1) NOT NULL DEFAULT '1',
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = 'Konfigurasi harga flat À La Carte (foto, video, dll)';

--
-- Dumping data for table `alacarte_config`
--

INSERT INTO
    `alacarte_config` (
        `id`,
        `package_code`,
        `label`,
        `price_min`,
        `price_max`,
        `description`,
        `display_order`,
        `active`,
        `updated_at`
    )
VALUES (
        1,
        'ac-fotohalf',
        'Foto Only (½ Hari)',
        3500000,
        5000000,
        'Sesi foto max ~75 siswa. Fotografer + fashion stylist.',
        1,
        1,
        '2026-05-04 02:13:18'
    ),
    (
        2,
        'ac-fotofull',
        'Foto Only (Full Day)',
        6000000,
        9000000,
        'Sesi foto 76–150+ siswa. Full team seharian.',
        2,
        1,
        '2026-05-04 02:13:18'
    ),
    (
        3,
        'ac-videod',
        'Drone Video',
        1500000,
        1500000,
        'Video drone 1–2 menit.',
        3,
        1,
        '2026-05-04 02:13:18'
    ),
    (
        4,
        'ac-videodoc',
        'Docudrama Video',
        3000000,
        3000000,
        'Video cerita angkatan 5–10 menit.',
        4,
        1,
        '2026-05-04 02:13:18'
    );

-- --------------------------------------------------------

--
-- Table structure for table `alacarte_factors`
--

CREATE TABLE `alacarte_factors` (
    `id` int NOT NULL,
    `package_code` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `factor` decimal(5, 3) NOT NULL,
    `min_per_book` int DEFAULT '0',
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `alacarte_factors`
--

INSERT INTO
    `alacarte_factors` (
        `id`,
        `package_code`,
        `factor`,
        `min_per_book`,
        `updated_at`
    )
VALUES (
        1,
        'ebook',
        0.800,
        0,
        '2026-04-23 13:26:55'
    ),
    (
        2,
        'editcetak',
        0.700,
        0,
        '2026-04-23 13:26:55'
    ),
    (
        3,
        'desain',
        0.600,
        0,
        '2026-04-23 13:26:55'
    ),
    (
        4,
        'cetakonly',
        0.480,
        0,
        '2026-04-23 13:26:55'
    );

-- --------------------------------------------------------

--
-- Table structure for table `bonus_fasilitas`
--

CREATE TABLE `bonus_fasilitas` (
    `id` int NOT NULL,
    `package_type` enum(
        'fullservice',
        'graduation',
        'alacarte'
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Tipe paket induk',
    `kategori` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'all',
    `label` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Judul bonus, mis: Studio Foto',
    `detail` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Deskripsi lengkap bonus',
    `display_order` int NOT NULL DEFAULT '0' COMMENT 'Urutan tampil',
    `active` tinyint(1) NOT NULL DEFAULT '1' COMMENT '1=aktif, 0=nonaktif',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = 'Master data Bonus & Fasilitas per tipe paket';

--
-- Dumping data for table `bonus_fasilitas`
--

INSERT INTO
    `bonus_fasilitas` (
        `id`,
        `package_type`,
        `kategori`,
        `label`,
        `detail`,
        `display_order`,
        `active`,
        `created_at`,
        `updated_at`
    )
VALUES (
        1,
        'fullservice',
        'all',
        'Studio Foto',
        'Free portable studio delivery, Fashion Stylist, Properti sesuai tema',
        1,
        1,
        '2026-04-22 11:29:58',
        '2026-04-22 11:29:58'
    ),
    (
        4,
        'fullservice',
        'all',
        'Pengiriman',
        'Gratis biaya pengiriman area Jabodetabek',
        4,
        1,
        '2026-04-22 11:29:58',
        '2026-04-27 05:52:59'
    ),
    (
        5,
        'graduation',
        'all',
        'Transportasi',
        'Gratis biaya transportasi area Jabodetabek',
        1,
        1,
        '2026-04-22 11:29:58',
        '2026-04-22 11:29:58'
    ),
    (
        6,
        'graduation',
        'all',
        'G-Drive',
        'File foto/video dikirim via Google Drive',
        2,
        1,
        '2026-04-22 11:29:58',
        '2026-04-22 11:29:58'
    ),
    (
        7,
        'graduation',
        'all',
        'Coverage',
        'Maksimal 4 jam liputan',
        3,
        1,
        '2026-04-22 11:29:58',
        '2026-04-22 11:29:58'
    ),
    (
        8,
        'alacarte',
        'all',
        'Konsultasi',
        'Konsultasi desain & konten gratis',
        1,
        1,
        '2026-04-22 11:29:58',
        '2026-04-22 11:29:58'
    ),
    (
        9,
        'alacarte',
        'all',
        'Revisi',
        'Revisi desain hingga 2 kali',
        2,
        1,
        '2026-04-22 11:29:58',
        '2026-04-22 11:29:58'
    );

-- --------------------------------------------------------

--
-- Table structure for table `cetak_base`
--

CREATE TABLE `cetak_base` (
    `id` int NOT NULL,
    `range_label` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `min_students` int NOT NULL,
    `max_students` int NOT NULL,
    `pages_count` int NOT NULL,
    `base_price` int NOT NULL,
    `description` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `cetak_base`
--

INSERT INTO
    `cetak_base` (
        `id`,
        `range_label`,
        `min_students`,
        `max_students`,
        `pages_count`,
        `base_price`,
        `description`,
        `created_at`,
        `updated_at`
    )
VALUES (
        94867,
        '20-29 siswa',
        20,
        29,
        30,
        108000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94868,
        '20-29 siswa',
        20,
        29,
        45,
        129000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94869,
        '20-29 siswa',
        20,
        29,
        60,
        151000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94870,
        '20-29 siswa',
        20,
        29,
        65,
        158000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94871,
        '20-29 siswa',
        20,
        29,
        75,
        172000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94872,
        '20-29 siswa',
        20,
        29,
        80,
        180000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94873,
        '20-29 siswa',
        20,
        29,
        90,
        193000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94874,
        '20-29 siswa',
        20,
        29,
        100,
        208000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94875,
        '20-29 siswa',
        20,
        29,
        110,
        221000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94876,
        '20-29 siswa',
        20,
        29,
        120,
        232000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94877,
        '20-29 siswa',
        20,
        29,
        135,
        252000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94878,
        '20-29 siswa',
        20,
        29,
        150,
        271000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94879,
        '20-29 siswa',
        20,
        29,
        160,
        283000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94880,
        '30–50 siswa',
        30,
        50,
        30,
        88000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94881,
        '30–50 siswa',
        30,
        50,
        45,
        109000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94882,
        '30–50 siswa',
        30,
        50,
        60,
        131000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94883,
        '30–50 siswa',
        30,
        50,
        65,
        138000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94884,
        '30–50 siswa',
        30,
        50,
        75,
        152000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94885,
        '30–50 siswa',
        30,
        50,
        80,
        160000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94886,
        '30–50 siswa',
        30,
        50,
        90,
        173000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94887,
        '30–50 siswa',
        30,
        50,
        100,
        188000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94888,
        '30–50 siswa',
        30,
        50,
        110,
        201000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94889,
        '30–50 siswa',
        30,
        50,
        120,
        213000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94890,
        '30–50 siswa',
        30,
        50,
        135,
        232000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94891,
        '30–50 siswa',
        30,
        50,
        150,
        251000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94892,
        '30–50 siswa',
        30,
        50,
        160,
        263000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94893,
        '51–75 siswa',
        51,
        75,
        30,
        83000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94894,
        '51–75 siswa',
        51,
        75,
        45,
        103000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94895,
        '51–75 siswa',
        51,
        75,
        60,
        123000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94896,
        '51–75 siswa',
        51,
        75,
        65,
        130000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94897,
        '51–75 siswa',
        51,
        75,
        75,
        143000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94898,
        '51–75 siswa',
        51,
        75,
        80,
        150000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94899,
        '51–75 siswa',
        51,
        75,
        90,
        163000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94900,
        '51–75 siswa',
        51,
        75,
        100,
        177000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94901,
        '51–75 siswa',
        51,
        75,
        110,
        189000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94902,
        '51–75 siswa',
        51,
        75,
        120,
        200000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94903,
        '51–75 siswa',
        51,
        75,
        135,
        218000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94904,
        '51–75 siswa',
        51,
        75,
        150,
        236000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94905,
        '51–75 siswa',
        51,
        75,
        160,
        248000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94906,
        '76–100 siswa',
        76,
        100,
        30,
        72000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94907,
        '76–100 siswa',
        76,
        100,
        45,
        89000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94908,
        '76–100 siswa',
        76,
        100,
        60,
        106000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94909,
        '76–100 siswa',
        76,
        100,
        65,
        112000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94910,
        '76–100 siswa',
        76,
        100,
        75,
        124000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94911,
        '76–100 siswa',
        76,
        100,
        80,
        130000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94912,
        '76–100 siswa',
        76,
        100,
        90,
        141000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94913,
        '76–100 siswa',
        76,
        100,
        100,
        153000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94914,
        '76–100 siswa',
        76,
        100,
        110,
        164000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94915,
        '76–100 siswa',
        76,
        100,
        120,
        175000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94916,
        '76–100 siswa',
        76,
        100,
        135,
        191000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94917,
        '76–100 siswa',
        76,
        100,
        150,
        207000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94918,
        '76–100 siswa',
        76,
        100,
        160,
        218000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94919,
        '101–125 siswa',
        101,
        125,
        30,
        63000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94920,
        '101–125 siswa',
        101,
        125,
        45,
        76000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94921,
        '101–125 siswa',
        101,
        125,
        60,
        90000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94922,
        '101–125 siswa',
        101,
        125,
        65,
        95000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94923,
        '101–125 siswa',
        101,
        125,
        75,
        105000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94924,
        '101–125 siswa',
        101,
        125,
        80,
        110000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94925,
        '101–125 siswa',
        101,
        125,
        90,
        121000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94926,
        '101–125 siswa',
        101,
        125,
        100,
        131000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94927,
        '101–125 siswa',
        101,
        125,
        110,
        141000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94928,
        '101–125 siswa',
        101,
        125,
        120,
        152000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94929,
        '101–125 siswa',
        101,
        125,
        135,
        167000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94930,
        '101–125 siswa',
        101,
        125,
        150,
        182000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94931,
        '101–125 siswa',
        101,
        125,
        160,
        192000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94932,
        '126–150 siswa',
        126,
        150,
        30,
        57000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94933,
        '126–150 siswa',
        126,
        150,
        45,
        71000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94934,
        '126–150 siswa',
        126,
        150,
        60,
        86000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94935,
        '126–150 siswa',
        126,
        150,
        65,
        91000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94936,
        '126–150 siswa',
        126,
        150,
        75,
        101000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94937,
        '126–150 siswa',
        126,
        150,
        80,
        106000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94938,
        '126–150 siswa',
        126,
        150,
        90,
        116000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94939,
        '126–150 siswa',
        126,
        150,
        100,
        126000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94940,
        '126–150 siswa',
        126,
        150,
        110,
        136000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94941,
        '126–150 siswa',
        126,
        150,
        120,
        146000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94942,
        '126–150 siswa',
        126,
        150,
        135,
        161000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94943,
        '126–150 siswa',
        126,
        150,
        150,
        176000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94944,
        '126–150 siswa',
        126,
        150,
        160,
        186000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94945,
        '151–175 siswa',
        151,
        175,
        30,
        53000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94946,
        '151–175 siswa',
        151,
        175,
        45,
        68000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94947,
        '151–175 siswa',
        151,
        175,
        60,
        83000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94948,
        '151–175 siswa',
        151,
        175,
        65,
        88000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94949,
        '151–175 siswa',
        151,
        175,
        75,
        97000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94950,
        '151–175 siswa',
        151,
        175,
        80,
        102000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94951,
        '151–175 siswa',
        151,
        175,
        90,
        112000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94952,
        '151–175 siswa',
        151,
        175,
        100,
        121000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94953,
        '151–175 siswa',
        151,
        175,
        110,
        130000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94954,
        '151–175 siswa',
        151,
        175,
        120,
        139000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94955,
        '151–175 siswa',
        151,
        175,
        135,
        153000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94956,
        '151–175 siswa',
        151,
        175,
        150,
        167000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94957,
        '151–175 siswa',
        151,
        175,
        160,
        176000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94958,
        '176–200 siswa',
        176,
        200,
        30,
        50000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94959,
        '176–200 siswa',
        176,
        200,
        45,
        63000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94960,
        '176–200 siswa',
        176,
        200,
        60,
        75000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94961,
        '176–200 siswa',
        176,
        200,
        65,
        80000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94962,
        '176–200 siswa',
        176,
        200,
        75,
        88000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94963,
        '176–200 siswa',
        176,
        200,
        80,
        93000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94964,
        '176–200 siswa',
        176,
        200,
        90,
        102000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94965,
        '176–200 siswa',
        176,
        200,
        100,
        110000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94966,
        '176–200 siswa',
        176,
        200,
        110,
        118000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94967,
        '176–200 siswa',
        176,
        200,
        120,
        126000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94968,
        '176–200 siswa',
        176,
        200,
        135,
        138000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94969,
        '176–200 siswa',
        176,
        200,
        150,
        150000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94970,
        '176–200 siswa',
        176,
        200,
        160,
        158000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94971,
        '201–225 siswa',
        201,
        225,
        30,
        47000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94972,
        '201–225 siswa',
        201,
        225,
        45,
        58000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94973,
        '201–225 siswa',
        201,
        225,
        60,
        70000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94974,
        '201–225 siswa',
        201,
        225,
        65,
        74000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94975,
        '201–225 siswa',
        201,
        225,
        75,
        82000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94976,
        '201–225 siswa',
        201,
        225,
        80,
        86000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94977,
        '201–225 siswa',
        201,
        225,
        90,
        94000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94978,
        '201–225 siswa',
        201,
        225,
        100,
        102000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94979,
        '201–225 siswa',
        201,
        225,
        110,
        109000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94980,
        '201–225 siswa',
        201,
        225,
        120,
        116000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94981,
        '201–225 siswa',
        201,
        225,
        135,
        127000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94982,
        '201–225 siswa',
        201,
        225,
        150,
        138000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94983,
        '201–225 siswa',
        201,
        225,
        160,
        145000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94984,
        '226–250 siswa',
        226,
        250,
        30,
        44000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94985,
        '226–250 siswa',
        226,
        250,
        45,
        55000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94986,
        '226–250 siswa',
        226,
        250,
        60,
        66000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94987,
        '226–250 siswa',
        226,
        250,
        65,
        70000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94988,
        '226–250 siswa',
        226,
        250,
        75,
        77000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94989,
        '226–250 siswa',
        226,
        250,
        80,
        81000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94990,
        '226–250 siswa',
        226,
        250,
        90,
        88000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94991,
        '226–250 siswa',
        226,
        250,
        100,
        96000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94992,
        '226–250 siswa',
        226,
        250,
        110,
        102000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94993,
        '226–250 siswa',
        226,
        250,
        120,
        109000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94994,
        '226–250 siswa',
        226,
        250,
        135,
        119000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94995,
        '226–250 siswa',
        226,
        250,
        150,
        129000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94996,
        '226–250 siswa',
        226,
        250,
        160,
        136000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94997,
        '251–275 siswa',
        251,
        275,
        30,
        42000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94998,
        '251–275 siswa',
        251,
        275,
        45,
        52000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        94999,
        '251–275 siswa',
        251,
        275,
        60,
        62000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95000,
        '251–275 siswa',
        251,
        275,
        65,
        65000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95001,
        '251–275 siswa',
        251,
        275,
        75,
        72000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95002,
        '251–275 siswa',
        251,
        275,
        80,
        76000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95003,
        '251–275 siswa',
        251,
        275,
        90,
        82000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95004,
        '251–275 siswa',
        251,
        275,
        100,
        89000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95005,
        '251–275 siswa',
        251,
        275,
        110,
        95000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95006,
        '251–275 siswa',
        251,
        275,
        120,
        101000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95007,
        '251–275 siswa',
        251,
        275,
        135,
        111000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95008,
        '251–275 siswa',
        251,
        275,
        150,
        121000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95009,
        '251–275 siswa',
        251,
        275,
        160,
        127000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95010,
        '276–300 siswa',
        276,
        300,
        30,
        39000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95011,
        '276–300 siswa',
        276,
        300,
        45,
        49000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95012,
        '276–300 siswa',
        276,
        300,
        60,
        58000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95013,
        '276–300 siswa',
        276,
        300,
        65,
        61000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95014,
        '276–300 siswa',
        276,
        300,
        75,
        67000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95015,
        '276–300 siswa',
        276,
        300,
        80,
        70000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95016,
        '276–300 siswa',
        276,
        300,
        90,
        77000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95017,
        '276–300 siswa',
        276,
        300,
        100,
        83000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95018,
        '276–300 siswa',
        276,
        300,
        110,
        89000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95019,
        '276–300 siswa',
        276,
        300,
        120,
        94000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95020,
        '276–300 siswa',
        276,
        300,
        135,
        103000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95021,
        '276–300 siswa',
        276,
        300,
        150,
        113000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95022,
        '276–300 siswa',
        276,
        300,
        160,
        119000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95023,
        '301–325 siswa',
        301,
        325,
        30,
        38000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95024,
        '301–325 siswa',
        301,
        325,
        45,
        46000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95025,
        '301–325 siswa',
        301,
        325,
        60,
        55000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95026,
        '301–325 siswa',
        301,
        325,
        65,
        58000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95027,
        '301–325 siswa',
        301,
        325,
        75,
        64000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95028,
        '301–325 siswa',
        301,
        325,
        80,
        67000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95029,
        '301–325 siswa',
        301,
        325,
        90,
        73000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95030,
        '301–325 siswa',
        301,
        325,
        100,
        79000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95031,
        '301–325 siswa',
        301,
        325,
        110,
        84000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95032,
        '301–325 siswa',
        301,
        325,
        120,
        89000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95033,
        '301–325 siswa',
        301,
        325,
        135,
        98000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95034,
        '301–325 siswa',
        301,
        325,
        150,
        107000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95035,
        '301–325 siswa',
        301,
        325,
        160,
        113000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95036,
        '326–350 siswa',
        326,
        350,
        30,
        37000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95037,
        '326–350 siswa',
        326,
        350,
        45,
        45000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95038,
        '326–350 siswa',
        326,
        350,
        60,
        54000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95039,
        '326–350 siswa',
        326,
        350,
        65,
        57000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95040,
        '326–350 siswa',
        326,
        350,
        75,
        63000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95041,
        '326–350 siswa',
        326,
        350,
        80,
        65000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95042,
        '326–350 siswa',
        326,
        350,
        90,
        71000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95043,
        '326–350 siswa',
        326,
        350,
        100,
        77000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95044,
        '326–350 siswa',
        326,
        350,
        110,
        82000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95045,
        '326–350 siswa',
        326,
        350,
        120,
        87000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95046,
        '326–350 siswa',
        326,
        350,
        135,
        96000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95047,
        '326–350 siswa',
        326,
        350,
        150,
        104000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95048,
        '326–350 siswa',
        326,
        350,
        160,
        110000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95049,
        '351–375 siswa',
        351,
        375,
        30,
        36000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95050,
        '351–375 siswa',
        351,
        375,
        45,
        44000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95051,
        '351–375 siswa',
        351,
        375,
        60,
        52000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95052,
        '351–375 siswa',
        351,
        375,
        65,
        55000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95053,
        '351–375 siswa',
        351,
        375,
        75,
        61000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95054,
        '351–375 siswa',
        351,
        375,
        80,
        64000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95055,
        '351–375 siswa',
        351,
        375,
        90,
        69000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95056,
        '351–375 siswa',
        351,
        375,
        100,
        75000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95057,
        '351–375 siswa',
        351,
        375,
        110,
        80000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95058,
        '351–375 siswa',
        351,
        375,
        120,
        85000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95059,
        '351–375 siswa',
        351,
        375,
        135,
        93000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95060,
        '351–375 siswa',
        351,
        375,
        150,
        102000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95061,
        '351–375 siswa',
        351,
        375,
        160,
        107000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95062,
        '376–400 siswa',
        376,
        400,
        30,
        35000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95063,
        '376–400 siswa',
        376,
        400,
        45,
        43000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95064,
        '376–400 siswa',
        376,
        400,
        60,
        51000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95065,
        '376–400 siswa',
        376,
        400,
        65,
        54000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95066,
        '376–400 siswa',
        376,
        400,
        75,
        59000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95067,
        '376–400 siswa',
        376,
        400,
        80,
        62000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95068,
        '376–400 siswa',
        376,
        400,
        90,
        67000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95069,
        '376–400 siswa',
        376,
        400,
        100,
        73000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95070,
        '376–400 siswa',
        376,
        400,
        110,
        78000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95071,
        '376–400 siswa',
        376,
        400,
        120,
        83000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95072,
        '376–400 siswa',
        376,
        400,
        135,
        91000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95073,
        '376–400 siswa',
        376,
        400,
        150,
        99000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95074,
        '376–400 siswa',
        376,
        400,
        160,
        104000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95075,
        '401–425 siswa',
        401,
        425,
        30,
        34000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95076,
        '401–425 siswa',
        401,
        425,
        45,
        42000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95077,
        '401–425 siswa',
        401,
        425,
        60,
        50000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95078,
        '401–425 siswa',
        401,
        425,
        65,
        53000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95079,
        '401–425 siswa',
        401,
        425,
        75,
        58000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95080,
        '401–425 siswa',
        401,
        425,
        80,
        61000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95081,
        '401–425 siswa',
        401,
        425,
        90,
        66000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95082,
        '401–425 siswa',
        401,
        425,
        100,
        72000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95083,
        '401–425 siswa',
        401,
        425,
        110,
        76000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95084,
        '401–425 siswa',
        401,
        425,
        120,
        81000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95085,
        '401–425 siswa',
        401,
        425,
        135,
        89000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95086,
        '401–425 siswa',
        401,
        425,
        150,
        97000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95087,
        '401–425 siswa',
        401,
        425,
        160,
        102000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95088,
        '426–450 siswa',
        426,
        450,
        30,
        34000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95089,
        '426–450 siswa',
        426,
        450,
        45,
        41000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95090,
        '426–450 siswa',
        426,
        450,
        60,
        49000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95091,
        '426–450 siswa',
        426,
        450,
        65,
        52000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95092,
        '426–450 siswa',
        426,
        450,
        75,
        57000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95093,
        '426–450 siswa',
        426,
        450,
        80,
        60000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95094,
        '426–450 siswa',
        426,
        450,
        90,
        65000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95095,
        '426–450 siswa',
        426,
        450,
        100,
        71000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95096,
        '426–450 siswa',
        426,
        450,
        110,
        75000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95097,
        '426–450 siswa',
        426,
        450,
        120,
        80000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95098,
        '426–450 siswa',
        426,
        450,
        135,
        88000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95099,
        '426–450 siswa',
        426,
        450,
        150,
        96000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95100,
        '426–450 siswa',
        426,
        450,
        160,
        101000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95101,
        '451–475 siswa',
        451,
        475,
        30,
        33000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95102,
        '451–475 siswa',
        451,
        475,
        45,
        41000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95103,
        '451–475 siswa',
        451,
        475,
        60,
        49000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95104,
        '451–475 siswa',
        451,
        475,
        65,
        51000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95105,
        '451–475 siswa',
        451,
        475,
        75,
        57000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95106,
        '451–475 siswa',
        451,
        475,
        80,
        59000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95107,
        '451–475 siswa',
        451,
        475,
        90,
        65000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95108,
        '451–475 siswa',
        451,
        475,
        100,
        70000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95109,
        '451–475 siswa',
        451,
        475,
        110,
        74000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95110,
        '451–475 siswa',
        451,
        475,
        120,
        79000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95111,
        '451–475 siswa',
        451,
        475,
        135,
        87000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95112,
        '451–475 siswa',
        451,
        475,
        150,
        95000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95113,
        '451–475 siswa',
        451,
        475,
        160,
        100000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95114,
        '476–500 siswa',
        476,
        500,
        30,
        33000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95115,
        '476–500 siswa',
        476,
        500,
        45,
        40000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95116,
        '476–500 siswa',
        476,
        500,
        60,
        48000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95117,
        '476–500 siswa',
        476,
        500,
        65,
        51000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95118,
        '476–500 siswa',
        476,
        500,
        75,
        56000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95119,
        '476–500 siswa',
        476,
        500,
        80,
        59000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95120,
        '476–500 siswa',
        476,
        500,
        90,
        64000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95121,
        '476–500 siswa',
        476,
        500,
        100,
        69000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95122,
        '476–500 siswa',
        476,
        500,
        110,
        73000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95123,
        '476–500 siswa',
        476,
        500,
        120,
        78000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95124,
        '476–500 siswa',
        476,
        500,
        135,
        86000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95125,
        '476–500 siswa',
        476,
        500,
        150,
        93000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    ),
    (
        95126,
        '476–500 siswa',
        476,
        500,
        160,
        99000,
        NULL,
        '2026-06-04 09:29:49',
        '2026-06-04 09:29:49'
    );

-- --------------------------------------------------------

--
-- Table structure for table `cetak_factors`
--

CREATE TABLE `cetak_factors` (
    `id` int NOT NULL,
    `package_type` enum('handy', 'minimal', 'large') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `factor` decimal(5, 3) NOT NULL,
    `description` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `cetak_factors`
--

INSERT INTO
    `cetak_factors` (
        `id`,
        `package_type`,
        `factor`,
        `description`,
        `updated_at`
    )
VALUES (
        1,
        'handy',
        1.000,
        'Faktor standar untuk Handy Book',
        '2026-04-06 05:03:41'
    ),
    (
        2,
        'minimal',
        1.020,
        'Faktor untuk Minimal Square Book',
        '2026-04-23 13:49:01'
    ),
    (
        3,
        'large',
        1.100,
        'Faktor untuk Large B4 Book',
        '2026-04-23 05:22:21'
    );

-- --------------------------------------------------------

--
-- Table structure for table `fullservice_pricing`
--

CREATE TABLE `fullservice_pricing` (
    `id` int NOT NULL,
    `package_type` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `min_students` int NOT NULL,
    `fs_type` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `max_students` int NOT NULL,
    `price_per_student` bigint NOT NULL,
    `pages` int DEFAULT '60',
    `active` tinyint DEFAULT '1',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `fullservice_pricing`
--

INSERT INTO
    `fullservice_pricing` (
        `id`,
        `package_type`,
        `min_students`,
        `fs_type`,
        `max_students`,
        `price_per_student`,
        `pages`,
        `active`,
        `created_at`,
        `updated_at`
    )
VALUES (
        115,
        'handy',
        30,
        NULL,
        50,
        465000,
        30,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        116,
        'handy',
        51,
        NULL,
        75,
        415000,
        30,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        117,
        'handy',
        76,
        NULL,
        100,
        370000,
        45,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        118,
        'handy',
        101,
        NULL,
        125,
        350000,
        55,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        119,
        'handy',
        126,
        NULL,
        150,
        335000,
        60,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        120,
        'handy',
        151,
        NULL,
        175,
        315000,
        65,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        121,
        'handy',
        176,
        NULL,
        200,
        295000,
        75,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        122,
        'handy',
        201,
        NULL,
        225,
        260000,
        80,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        123,
        'handy',
        226,
        NULL,
        250,
        250000,
        80,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        124,
        'handy',
        251,
        NULL,
        275,
        240000,
        90,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        125,
        'handy',
        276,
        NULL,
        300,
        230000,
        100,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        126,
        'handy',
        300,
        NULL,
        325,
        220000,
        100,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        127,
        'handy',
        326,
        NULL,
        350,
        210000,
        120,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        128,
        'handy',
        351,
        NULL,
        375,
        200000,
        120,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        129,
        'handy',
        376,
        NULL,
        400,
        190000,
        135,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        130,
        'handy',
        401,
        NULL,
        425,
        185000,
        135,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        131,
        'handy',
        426,
        NULL,
        450,
        165000,
        145,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        132,
        'handy',
        451,
        NULL,
        475,
        175000,
        150,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        133,
        'handy',
        476,
        NULL,
        500,
        150000,
        160,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        134,
        'minimal',
        30,
        NULL,
        50,
        450000,
        30,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        135,
        'minimal',
        51,
        NULL,
        75,
        400000,
        30,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        136,
        'minimal',
        76,
        NULL,
        100,
        355000,
        45,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        137,
        'minimal',
        101,
        NULL,
        125,
        335000,
        55,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        138,
        'minimal',
        126,
        NULL,
        150,
        320000,
        60,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        139,
        'minimal',
        151,
        NULL,
        175,
        300000,
        65,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        140,
        'minimal',
        176,
        NULL,
        200,
        280000,
        75,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        141,
        'minimal',
        201,
        NULL,
        225,
        245000,
        80,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        142,
        'minimal',
        226,
        NULL,
        250,
        235000,
        80,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        143,
        'minimal',
        251,
        NULL,
        275,
        240000,
        90,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        144,
        'minimal',
        276,
        NULL,
        300,
        215000,
        100,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        145,
        'minimal',
        300,
        NULL,
        325,
        205000,
        100,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        146,
        'minimal',
        326,
        NULL,
        350,
        195000,
        120,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        147,
        'minimal',
        351,
        NULL,
        375,
        185000,
        120,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        148,
        'minimal',
        376,
        NULL,
        400,
        180000,
        135,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        149,
        'minimal',
        401,
        NULL,
        425,
        170000,
        135,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        150,
        'minimal',
        426,
        NULL,
        450,
        160000,
        145,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        151,
        'minimal',
        451,
        NULL,
        475,
        150000,
        150,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        152,
        'minimal',
        476,
        NULL,
        500,
        140000,
        160,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        153,
        'large',
        30,
        NULL,
        50,
        480000,
        30,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        154,
        'large',
        51,
        NULL,
        75,
        430000,
        30,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        155,
        'large',
        76,
        NULL,
        100,
        405000,
        45,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        156,
        'large',
        101,
        NULL,
        125,
        365000,
        55,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        157,
        'large',
        126,
        NULL,
        150,
        350000,
        60,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        158,
        'large',
        151,
        NULL,
        175,
        330000,
        65,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        159,
        'large',
        176,
        NULL,
        200,
        310000,
        75,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        160,
        'large',
        201,
        NULL,
        225,
        275000,
        80,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        161,
        'large',
        226,
        NULL,
        250,
        265000,
        80,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        162,
        'large',
        251,
        NULL,
        275,
        255000,
        90,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        163,
        'large',
        276,
        NULL,
        300,
        245000,
        100,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        164,
        'large',
        300,
        NULL,
        325,
        235000,
        100,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        165,
        'large',
        326,
        NULL,
        350,
        225000,
        120,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        166,
        'large',
        351,
        NULL,
        375,
        215000,
        120,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        167,
        'large',
        376,
        NULL,
        400,
        205000,
        135,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        168,
        'large',
        401,
        NULL,
        425,
        195000,
        135,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        169,
        'large',
        426,
        NULL,
        450,
        175000,
        145,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        170,
        'large',
        451,
        NULL,
        475,
        165000,
        150,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        171,
        'large',
        476,
        NULL,
        500,
        155000,
        160,
        1,
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    );

-- --------------------------------------------------------

--
-- Table structure for table `graduation_addons`
--

CREATE TABLE `graduation_addons` (
    `id` int NOT NULL,
    `addon_key` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `name` varchar(150) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `price` int NOT NULL,
    `addon_type` enum('addon', 'cetak', 'service') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'addon',
    `unit` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `description` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `graduation_addons`
--

INSERT INTO
    `graduation_addons` (
        `id`,
        `addon_key`,
        `name`,
        `price`,
        `addon_type`,
        `unit`,
        `description`,
        `created_at`,
        `updated_at`
    )
VALUES (
        40,
        'gbooth_add',
        'Tambah 1 Jam Photobooth/360',
        500000,
        'addon',
        NULL,
        NULL,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        41,
        'gphoto_add',
        'Tambah 1 Fotografer',
        1250000,
        'addon',
        NULL,
        NULL,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        42,
        'gvideo_add',
        'Tambah 1 Videografer',
        1100000,
        'addon',
        NULL,
        NULL,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        43,
        'gwork_add',
        'Tambah 1 Jam Kerja/Orang',
        350000,
        'addon',
        NULL,
        NULL,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    );

-- --------------------------------------------------------

--
-- Table structure for table `graduation_cetak`
--

CREATE TABLE `graduation_cetak` (
    `id` int NOT NULL,
    `cetak_key` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `name` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `price_per_unit` bigint NOT NULL,
    `description` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `active` tinyint DEFAULT '1',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `graduation_cetak`
--

INSERT INTO
    `graduation_cetak` (
        `id`,
        `cetak_key`,
        `name`,
        `price_per_unit`,
        `description`,
        `active`,
        `created_at`,
        `updated_at`
    )
VALUES (
        29,
        'g4r',
        'Cetak Foto 4R',
        4000,
        NULL,
        1,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        30,
        'g8r',
        'Cetak Foto 8R',
        8000,
        NULL,
        1,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        31,
        'g10r',
        'Cetak Foto 10R',
        15000,
        NULL,
        1,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        32,
        'g12r',
        'Cetak Foto 12R',
        20000,
        NULL,
        1,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    );

-- --------------------------------------------------------

--
-- Table structure for table `jasa_termasuk`
--

CREATE TABLE `jasa_termasuk` (
    `id` int NOT NULL,
    `package_type` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `kategori` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'all',
    `label` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `detail` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `display_order` int DEFAULT '0',
    `active` tinyint(1) DEFAULT '1',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `jasa_termasuk`
--

INSERT INTO
    `jasa_termasuk` (
        `id`,
        `package_type`,
        `kategori`,
        `label`,
        `detail`,
        `display_order`,
        `active`,
        `created_at`
    )
VALUES (
        1,
        'fullservice',
        'all',
        'Foto Produksi (Personal, Konsep, Konten)',
        'ajkshdkasd',
        1,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        2,
        'fullservice',
        'all',
        'Desain Cover & Layout',
        'qwebqwhe',
        2,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        3,
        'fullservice',
        'all',
        'Editing Foto Terpilih',
        'qweqwoieu',
        3,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        4,
        'alacarte',
        'ebook',
        'Foto Produksi',
        NULL,
        1,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        5,
        'alacarte',
        'ebook',
        'Editing',
        NULL,
        2,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        6,
        'alacarte',
        'ebook',
        'Desain & Layout (Output: File Digital)',
        NULL,
        3,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        7,
        'alacarte',
        'editcetak',
        'Editing Foto Terpilih',
        NULL,
        1,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        8,
        'alacarte',
        'editcetak',
        'Desain & Layout',
        NULL,
        2,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        9,
        'alacarte',
        'editcetak',
        'Cetak & Kirim',
        NULL,
        3,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        10,
        'alacarte',
        'fotohalf',
        'Sesi Foto Produksi',
        NULL,
        1,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        11,
        'alacarte',
        'fotohalf',
        'Fashion Stylist',
        NULL,
        2,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        12,
        'alacarte',
        'fotofull',
        'Sesi Foto Produksi',
        NULL,
        1,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        13,
        'alacarte',
        'fotofull',
        'Fashion Stylist',
        NULL,
        2,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        14,
        'alacarte',
        'videod',
        'Video Drone 1–2 menit',
        NULL,
        1,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        15,
        'alacarte',
        'videodoc',
        'Video Cerita Angkatan 5–10 menit',
        NULL,
        1,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        16,
        'alacarte',
        'desain',
        'Layout & Desain Buku (Klien sediakan konten)',
        NULL,
        1,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        17,
        'alacarte',
        'cetakonly',
        'Cetak & Pengiriman (File siap cetak dari klien)',
        NULL,
        1,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        18,
        'graduation',
        'all',
        'Dokumentasi Wisuda (Foto & Video sesuai paket)',
        NULL,
        1,
        1,
        '2026-04-28 07:31:01'
    ),
    (
        19,
        'alacarte',
        'ac-ebook',
        'lhljh',
        NULL,
        99,
        1,
        '2026-04-28 07:46:42'
    ),
    (
        20,
        'graduation',
        'gphv',
        'testtt data',
        NULL,
        99,
        1,
        '2026-04-28 07:58:42'
    );

-- --------------------------------------------------------

--
-- Table structure for table `overhead`
--

CREATE TABLE `overhead` (
    `id` int NOT NULL,
    `category` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `amount` int NOT NULL DEFAULT '0',
    `description` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `overhead`
--

INSERT INTO
    `overhead` (
        `id`,
        `category`,
        `amount`,
        `description`,
        `created_at`,
        `updated_at`
    )
VALUES (
        350,
        'designer',
        19500000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    ),
    (
        351,
        'marketing',
        5500000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    ),
    (
        352,
        'creative',
        7000000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    ),
    (
        353,
        'pm',
        8000000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    ),
    (
        354,
        'sosmed',
        7000000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    ),
    (
        355,
        'freelance',
        4000000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    ),
    (
        356,
        'operasional',
        3036000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    ),
    (
        357,
        'sewakantor',
        6250000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    ),
    (
        358,
        'biayaads',
        2000000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    ),
    (
        359,
        'entertaiment',
        1000000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    ),
    (
        360,
        'subscribe(web,drive,ai,dll)',
        1250000,
        NULL,
        '2026-06-04 09:33:42',
        '2026-06-04 09:33:42'
    );

-- --------------------------------------------------------

--
-- Table structure for table `overhead_total`
--

CREATE TABLE `overhead_total` (
    `id` int NOT NULL,
    `total_amount` bigint NOT NULL DEFAULT '0',
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `overhead_total`
--

INSERT INTO
    `overhead_total` (
        `id`,
        `total_amount`,
        `updated_at`
    )
VALUES (
        2,
        76786700,
        '2026-04-16 03:12:16'
    );

-- --------------------------------------------------------

--
-- Table structure for table `packages_alacarte`
--

CREATE TABLE `packages_alacarte` (
    `id` int NOT NULL,
    `code` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `name` varchar(150) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `description` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `price_type` enum(
        'per_siswa',
        'flat_range',
        'flat_fixed'
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `price_min` int DEFAULT NULL,
    `price_max` int DEFAULT NULL,
    `factor` decimal(5, 3) DEFAULT NULL,
    `margin_target` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `includes` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin,
    `excludes` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin,
    `display_order` int DEFAULT '0',
    `is_featured` tinyint(1) DEFAULT '0',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_general_ci;

--
-- Dumping data for table `packages_alacarte`
--

INSERT INTO
    `packages_alacarte` (
        `id`,
        `code`,
        `name`,
        `description`,
        `price_type`,
        `price_min`,
        `price_max`,
        `factor`,
        `margin_target`,
        `includes`,
        `excludes`,
        `display_order`,
        `is_featured`,
        `created_at`,
        `updated_at`
    )
VALUES (
        1,
        'ebook',
        'E-Book Package',
        'Foto+Editing+Desain, output file digital. Tanpa cetak fisik.',
        'per_siswa',
        NULL,
        NULL,
        0.680,
        '62–68%',
        NULL,
        NULL,
        0,
        1,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        3,
        'editcetak',
        'Edit+Desain+Cetak',
        'Klien bawa foto sendiri. Parama handle editing, layout, cetak & kirim.',
        'per_siswa',
        NULL,
        NULL,
        0.580,
        '55–62%',
        NULL,
        NULL,
        0,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        4,
        'fotohalf',
        'Foto Only (½ Hari)',
        'Sesi foto max ~75 siswa. Fotografer + fashion stylist.',
        'flat_range',
        3500000,
        5000000,
        NULL,
        '55–65%',
        NULL,
        NULL,
        0,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        5,
        'fotofull',
        'Foto Only (Full Day)',
        'Sesi foto 76–150+ siswa. Full team seharian.',
        'flat_range',
        6000000,
        9000000,
        NULL,
        '55–65%',
        NULL,
        NULL,
        0,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        6,
        'videodrone',
        'Drone Video',
        'Video drone 1–2 menit.',
        'flat_fixed',
        1500000,
        1500000,
        NULL,
        NULL,
        NULL,
        NULL,
        0,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        7,
        'videodoc',
        'Docudrama Video',
        'Video cerita angkatan 5–10 menit.',
        'flat_fixed',
        3000000,
        3000000,
        NULL,
        NULL,
        NULL,
        NULL,
        0,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        8,
        'desain',
        'Desain Only',
        'Klien bawa semua konten. Parama hanya layout buku.',
        'per_siswa',
        NULL,
        NULL,
        0.550,
        '55–65%',
        NULL,
        NULL,
        0,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    ),
    (
        9,
        'cetakonly',
        'Cetak Only',
        'Klien sudah punya file siap cetak. Parama cetak & kirim saja.',
        'per_siswa',
        NULL,
        NULL,
        0.350,
        '30–45%',
        NULL,
        NULL,
        0,
        0,
        '2026-04-06 05:03:41',
        '2026-04-06 05:03:41'
    );

-- --------------------------------------------------------

--
-- Table structure for table `packages_config`
--

CREATE TABLE `packages_config` (
    `id` int UNSIGNED NOT NULL,
    `package_code` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Kode unik (cth: fs-handy, ac-fotohalf)',
    `label` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Nama Tampilan',
    `package_type` enum(
        'fs',
        'alc_factor',
        'alc_flat'
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'alc_flat' COMMENT 'Jenis kalkulasi paket',
    `calc_key` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Key untuk lookup di array FS atau ALC_F (handy, minimal, ebook, dll)',
    `by_siswa` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Apakah dihitung per jumlah siswa',
    `price_min` int UNSIGNED NOT NULL DEFAULT '0' COMMENT 'Harga min (hanya untuk alc_flat)',
    `price_max` int UNSIGNED NOT NULL DEFAULT '0' COMMENT 'Harga maks (hanya untuk alc_flat)',
    `min_per_buku` int UNSIGNED NOT NULL DEFAULT '0' COMMENT 'Min harga per buku (hanya untuk desain/cetakonly)',
    `description` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Keterangan',
    `display_order` tinyint UNSIGNED NOT NULL DEFAULT '0',
    `active` tinyint(1) NOT NULL DEFAULT '1',
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `packages_config`
--

INSERT INTO
    `packages_config` (
        `id`,
        `package_code`,
        `label`,
        `package_type`,
        `calc_key`,
        `by_siswa`,
        `price_min`,
        `price_max`,
        `min_per_buku`,
        `description`,
        `display_order`,
        `active`,
        `updated_at`
    )
VALUES (
        1,
        'fs-handy',
        'Full Service — Handy Book A4+',
        'fs',
        'handy',
        1,
        0,
        0,
        0,
        NULL,
        1,
        1,
        '2026-05-04 02:09:58'
    ),
    (
        2,
        'fs-minimal',
        'Full Service — Minimal Book SQ',
        'fs',
        'minimal',
        1,
        0,
        0,
        0,
        NULL,
        2,
        1,
        '2026-05-04 02:09:58'
    ),
    (
        3,
        'fs-large',
        'Full Service — Large Book B4',
        'fs',
        'large',
        1,
        0,
        0,
        0,
        NULL,
        3,
        1,
        '2026-05-04 02:09:58'
    ),
    (
        4,
        'ac-ebook',
        'À La Carte — E-Book Package',
        'alc_factor',
        'ebook',
        1,
        0,
        0,
        0,
        NULL,
        4,
        1,
        '2026-05-04 02:09:58'
    ),
    (
        5,
        'ac-editcetak',
        'À La Carte — Edit+Desain+Cetak',
        'alc_factor',
        'editcetak',
        1,
        0,
        0,
        0,
        NULL,
        5,
        1,
        '2026-05-04 02:09:58'
    ),
    (
        6,
        'ac-desain',
        'À La Carte — Desain Only',
        'alc_factor',
        'desain',
        1,
        0,
        0,
        50000,
        NULL,
        6,
        1,
        '2026-05-04 02:09:58'
    ),
    (
        7,
        'ac-cetakonly',
        'À La Carte — Cetak Only',
        'alc_factor',
        'cetakonly',
        1,
        0,
        0,
        30000,
        NULL,
        7,
        1,
        '2026-05-04 02:09:58'
    ),
    (
        8,
        'ac-fotohalf',
        'À La Carte — Foto Only (1 Kelas | Tanpa Studio Delivery & Properti)',
        'alc_flat',
        NULL,
        0,
        3800000,
        3800000,
        0,
        '1 Kelas',
        8,
        1,
        '2026-05-04 07:51:08'
    ),
    (
        9,
        'ac-fotofull',
        'À La Carte — Foto Only (1 Kelas & Full Service)',
        'alc_flat',
        NULL,
        0,
        4950000,
        4950000,
        0,
        '1 Kelas',
        9,
        1,
        '2026-05-04 07:50:47'
    ),
    (
        10,
        'ac-videod',
        'À La Carte — Drone Video',
        'alc_flat',
        NULL,
        0,
        1500000,
        1500000,
        0,
        NULL,
        10,
        1,
        '2026-05-04 02:09:58'
    ),
    (
        11,
        'ac-videodoc',
        'À La Carte — Docudrama Video',
        'alc_flat',
        NULL,
        0,
        3000000,
        3000000,
        0,
        NULL,
        11,
        1,
        '2026-05-04 02:09:58'
    );

-- --------------------------------------------------------

--
-- Table structure for table `packages_fullservice`
--

CREATE TABLE `packages_fullservice` (
    `id` int NOT NULL,
    `package_type` enum('handy', 'minimal', 'large') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `min_students` int NOT NULL,
    `max_students` int NOT NULL,
    `price_per_book` int NOT NULL,
    `max_pages` int NOT NULL,
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `packages_fullservice`
--

INSERT INTO
    `packages_fullservice` (
        `id`,
        `package_type`,
        `min_students`,
        `max_students`,
        `price_per_book`,
        `max_pages`,
        `created_at`,
        `updated_at`
    )
VALUES (
        13339,
        'handy',
        30,
        50,
        575000,
        20,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13340,
        'handy',
        51,
        75,
        485000,
        30,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13341,
        'handy',
        76,
        100,
        395000,
        40,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13342,
        'handy',
        101,
        125,
        360000,
        50,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13343,
        'handy',
        126,
        150,
        335000,
        60,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13344,
        'handy',
        151,
        175,
        325000,
        65,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13345,
        'handy',
        176,
        200,
        310000,
        75,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13346,
        'handy',
        201,
        225,
        270000,
        80,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13347,
        'handy',
        226,
        250,
        260000,
        80,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13348,
        'handy',
        251,
        275,
        250000,
        90,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13349,
        'handy',
        276,
        300,
        240000,
        100,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13350,
        'handy',
        300,
        325,
        230000,
        100,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13351,
        'handy',
        326,
        350,
        225000,
        120,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13352,
        'handy',
        351,
        375,
        220000,
        120,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13353,
        'handy',
        376,
        400,
        210000,
        125,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13354,
        'handy',
        401,
        425,
        205000,
        130,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13355,
        'handy',
        426,
        450,
        200000,
        130,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13356,
        'handy',
        451,
        475,
        195000,
        140,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13357,
        'handy',
        476,
        500,
        190000,
        140,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13358,
        'minimal',
        30,
        50,
        585000,
        20,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13359,
        'minimal',
        51,
        75,
        490000,
        30,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13360,
        'minimal',
        76,
        100,
        400000,
        40,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13361,
        'minimal',
        101,
        125,
        365000,
        50,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13362,
        'minimal',
        126,
        150,
        340000,
        60,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13363,
        'minimal',
        151,
        175,
        330000,
        65,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13364,
        'minimal',
        176,
        200,
        280000,
        75,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13365,
        'minimal',
        201,
        225,
        245000,
        80,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13366,
        'minimal',
        226,
        250,
        235000,
        80,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13367,
        'minimal',
        251,
        275,
        230000,
        90,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13368,
        'minimal',
        276,
        300,
        225000,
        100,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13369,
        'minimal',
        300,
        325,
        220000,
        100,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13370,
        'minimal',
        326,
        350,
        210000,
        120,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13371,
        'minimal',
        351,
        375,
        205000,
        120,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13372,
        'minimal',
        376,
        400,
        200000,
        125,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13373,
        'minimal',
        401,
        425,
        195000,
        130,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13374,
        'minimal',
        426,
        450,
        190000,
        130,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13375,
        'minimal',
        451,
        475,
        185000,
        140,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13376,
        'minimal',
        476,
        500,
        180000,
        140,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13377,
        'large',
        30,
        50,
        590000,
        30,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13378,
        'large',
        51,
        75,
        500000,
        30,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13379,
        'large',
        76,
        100,
        410000,
        45,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13380,
        'large',
        101,
        125,
        375000,
        55,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13381,
        'large',
        126,
        150,
        350000,
        60,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13382,
        'large',
        151,
        175,
        340000,
        65,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13383,
        'large',
        176,
        200,
        325000,
        75,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13384,
        'large',
        201,
        225,
        285000,
        80,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13385,
        'large',
        226,
        250,
        275000,
        80,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13386,
        'large',
        251,
        275,
        265000,
        90,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13387,
        'large',
        276,
        300,
        255000,
        100,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13388,
        'large',
        300,
        325,
        245000,
        100,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13389,
        'large',
        326,
        350,
        235000,
        110,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13390,
        'large',
        351,
        375,
        225000,
        120,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13391,
        'large',
        376,
        400,
        215000,
        135,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13392,
        'large',
        401,
        425,
        210000,
        135,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13393,
        'large',
        426,
        450,
        200000,
        140,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13394,
        'large',
        451,
        475,
        195000,
        140,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    ),
    (
        13395,
        'large',
        476,
        500,
        190000,
        140,
        '2026-06-04 09:28:18',
        '2026-06-04 09:28:18'
    );

-- --------------------------------------------------------

--
-- Table structure for table `packages_fullservice_backup`
--

CREATE TABLE `packages_fullservice_backup` (
    `id` int NOT NULL DEFAULT '0',
    `package_type` enum('handy', 'minimal', 'large') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `min_students` int NOT NULL,
    `max_students` int NOT NULL,
    `price_per_book` int NOT NULL,
    `max_pages` int NOT NULL,
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `packages_fullservice_backup`
--

INSERT INTO
    `packages_fullservice_backup` (
        `id`,
        `package_type`,
        `min_students`,
        `max_students`,
        `price_per_book`,
        `max_pages`,
        `created_at`,
        `updated_at`
    )
VALUES (
        1,
        'handy',
        25,
        50,
        399000,
        70,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        2,
        'handy',
        51,
        75,
        389000,
        75,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        3,
        'handy',
        76,
        100,
        379000,
        80,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        4,
        'handy',
        101,
        150,
        369000,
        90,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        5,
        'minimal',
        25,
        50,
        349000,
        65,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        6,
        'minimal',
        51,
        75,
        339000,
        70,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        7,
        'minimal',
        76,
        100,
        329000,
        75,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        8,
        'minimal',
        101,
        150,
        319000,
        85,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        9,
        'large',
        25,
        50,
        449000,
        75,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        10,
        'large',
        51,
        75,
        439000,
        85,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        11,
        'large',
        76,
        100,
        429000,
        95,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    ),
    (
        12,
        'large',
        101,
        150,
        419000,
        105,
        '2026-04-06 05:02:47',
        '2026-04-06 05:02:47'
    );

-- --------------------------------------------------------

--
-- Table structure for table `packages_graduation`
--

CREATE TABLE `packages_graduation` (
    `id` int NOT NULL,
    `package_key` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `name` varchar(150) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `desc` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `color` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `price` int NOT NULL,
    `color_scheme` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `is_featured` tinyint(1) DEFAULT '0',
    `transport_included` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `display_order` int DEFAULT '0',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `packages_graduation`
--

INSERT INTO
    `packages_graduation` (
        `id`,
        `package_key`,
        `name`,
        `desc`,
        `color`,
        `price`,
        `color_scheme`,
        `is_featured`,
        `transport_included`,
        `display_order`,
        `created_at`,
        `updated_at`
    )
VALUES (
        55,
        'gphv',
        'Photo & Video',
        '2 Fotografer + 1 Videografer, 50 foto edited, video cinematic 2–4 mnt, G-Drive, 4 jam coverage, transport jabodetabek',
        '',
        4800000,
        NULL,
        0,
        NULL,
        0,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        56,
        'gvideo',
        'Video Only',
        '1 Videografer, video cinematic 2–5 mnt, G-Drive, 4 jam coverage, transport jabodetabek',
        '',
        2500000,
        NULL,
        0,
        NULL,
        1,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        57,
        'gphoto',
        'Photo Only',
        '2 Fotografer, 100 foto edited, G-Drive, 4 jam coverage, transport jabodetabek',
        '',
        2750000,
        NULL,
        0,
        NULL,
        2,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        58,
        'gbooth',
        'Photo Booth',
        '1–2 Crew profesional, backdrop wisuda, lighting studio, Selfiebox Machine, unlimited print 4R, max 3 jam, softcopy + QR Code realtime, transport jabodetabek',
        '',
        3850000,
        NULL,
        0,
        NULL,
        3,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        59,
        'g360',
        'Glamation 360°',
        '1–2 Crew profesional, MP4, LCD 50in preview, GoPro/iPhone 12 Pro, overlay design free, max 3 jam, QR Code realtime, transport jabodetabek',
        '',
        4100000,
        NULL,
        0,
        NULL,
        4,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    ),
    (
        60,
        'gcomplete',
        'Complete Package',
        'Photo (2 foto, 100 edited) + Video (1 videografer, cinematic 2–4 mnt) + Photo Booth (unlimited print 4R, max 3 jam, QR Code), transport jabodetabek',
        '',
        7750000,
        NULL,
        0,
        NULL,
        5,
        '2026-04-30 09:39:45',
        '2026-04-30 09:39:45'
    );

-- --------------------------------------------------------

--
-- Table structure for table `payment_terms`
--

CREATE TABLE `payment_terms` (
    `id` int NOT NULL,
    `term_key` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `term_name` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `description` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `deposit` int DEFAULT '0',
    `color` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `active` tinyint DEFAULT '1',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `payment_terms`
--

INSERT INTO
    `payment_terms` (
        `id`,
        `term_key`,
        `term_name`,
        `description`,
        `deposit`,
        `color`,
        `active`,
        `created_at`,
        `updated_at`
    )
VALUES (
        16,
        'pt_full',
        'Pembayaran Penuh',
        'Bayar 100% sebelum pekerjaan dimulai — diskon 5% untuk cash/transfer hkg',
        100,
        '#2ecc71',
        1,
        '2026-04-16 03:33:31',
        '2026-04-16 03:33:31'
    ),
    (
        17,
        'pt_dp50',
        'DP 50% tes',
        'Bayar 50% di awal, sisa 50% setelah serah terima. Standar untuk mayoritas klien.',
        50,
        '#3498db',
        1,
        '2026-04-16 03:33:31',
        '2026-04-16 03:33:31'
    ),
    (
        18,
        'pt_dp30',
        'DP 30%',
        'Bayar 30% uang muka, 40% setelah produksi, 30% sebelum pengiriman. Untuk project besar.',
        30,
        '#E74C3C',
        1,
        '2026-04-16 03:33:31',
        '2026-04-16 03:33:31'
    ),
    (
        19,
        'pt_dp20',
        'DP 20%',
        'Untuk klien korporat/kontrak jangka panjang. Pembayaran bertahap: 20% + 40% + 40%.',
        20,
        '#F39C12',
        1,
        '2026-04-16 03:33:31',
        '2026-04-16 03:33:31'
    ),
    (
        20,
        'pt_installment',
        'Cicilan (4x)',
        '4x cicilan bulanan tanpa bunga. Tersedia untuk nominal >Rp 10 juta.',
        25,
        '#9B59B6',
        1,
        '2026-04-16 03:33:31',
        '2026-04-16 03:33:31'
    );

-- --------------------------------------------------------

--
-- Table structure for table `pdf_ketentuan`
--

CREATE TABLE `pdf_ketentuan` (
    `id` int NOT NULL,
    `package_type` enum(
        'fullservice',
        'graduation',
        'alacarte'
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `text_content` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `display_order` int DEFAULT '0',
    `active` tinyint(1) DEFAULT '1',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `pdf_ketentuan`
--

INSERT INTO
    `pdf_ketentuan` (
        `id`,
        `package_type`,
        `text_content`,
        `display_order`,
        `active`,
        `created_at`,
        `updated_at`
    )
VALUES (
        1,
        'fullservice',
        'Harga berlaku untuk minimal {siswa} pemesan Buku Tahunan.',
        1,
        1,
        '2026-04-22 12:18:02',
        '2026-04-22 12:18:02'
    ),
    (
        2,
        'fullservice',
        'Harga bersifat penawaran dan dapat berubah sesuai kesepakatan.',
        2,
        1,
        '2026-04-22 12:18:02',
        '2026-04-22 12:18:02'
    ),
    (
        3,
        'graduation',
        'Harga berlaku untuk event yang telah disepakati.',
        1,
        1,
        '2026-04-22 12:18:02',
        '2026-04-22 12:18:02'
    ),
    (
        4,
        'graduation',
        'Harga bersifat penawaran dan dapat berubah sesuai kesepakatan.',
        2,
        1,
        '2026-04-22 12:18:02',
        '2026-04-22 12:18:02'
    ),
    (
        5,
        'alacarte',
        'Harga berlaku sesuai spesifikasi yang tercantum.',
        1,
        1,
        '2026-04-22 12:18:02',
        '2026-04-22 12:18:02'
    ),
    (
        6,
        'alacarte',
        'Harga bersifat penawaran dan dapat berubah sesuai kesepakatan.',
        2,
        1,
        '2026-04-22 12:18:02',
        '2026-04-22 12:18:02'
    );

-- --------------------------------------------------------

--
-- Table structure for table `penawaran`
--

CREATE TABLE `penawaran` (
    `id` int NOT NULL,
    `nama_klien` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `paket` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT '',
    `harga` bigint DEFAULT '0',
    `harga_sebelum_diskon` bigint DEFAULT '0',
    `jumlah_siswa` int DEFAULT '0',
    `catatan` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `status` enum(
        'pending',
        'nego',
        'deal',
        'gagal'
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'pending',
    `added_by` int DEFAULT NULL,
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `penawaran`
--

INSERT INTO
    `penawaran` (
        `id`,
        `nama_klien`,
        `paket`,
        `harga`,
        `harga_sebelum_diskon`,
        `jumlah_siswa`,
        `catatan`,
        `status`,
        `added_by`,
        `created_at`,
        `updated_at`
    )
VALUES (
        1,
        'sekolah SMPN 1 Tangerang',
        'Full Service — Handy Book A4+',
        84000000,
        0,
        100,
        'addons: binding,covbahan,klip,popup,cbox1,cbox2',
        'deal',
        NULL,
        '2026-04-06 02:20:30',
        '2026-04-23 10:39:47'
    ),
    (
        2,
        'Test School',
        'À La Carte — E-Book Package',
        27650000,
        0,
        100,
        '',
        'deal',
        NULL,
        '2026-04-15 22:05:27',
        '2026-04-17 03:48:10'
    ),
    (
        3,
        'smp1',
        'Full Service — Handy Book A4+',
        37000000,
        0,
        100,
        '',
        'nego',
        NULL,
        '2026-04-15 22:05:27',
        '2026-04-15 22:05:27'
    ),
    (
        5,
        'sekolah B',
        'Full Service — Handy Book A4+',
        43225000,
        0,
        70,
        '',
        'pending',
        2,
        '2026-04-21 02:49:36',
        '2026-04-23 07:12:54'
    ),
    (
        6,
        'test graduation',
        'Graduation — Photo & Video',
        5200000,
        0,
        100,
        'addons: g4r:100',
        'pending',
        2,
        '2026-04-23 06:56:19',
        '2026-04-29 03:01:28'
    ),
    (
        7,
        'testtt erere',
        'Graduation — Photo & Video',
        8000000,
        0,
        100,
        'addons: gbooth_add,gphoto_add,gvideo_add,gwork_add',
        'pending',
        1,
        '2026-04-28 08:49:29',
        '2026-04-28 09:21:50'
    ),
    (
        8,
        'SMA 70',
        'Full Service — Handy Book A4+',
        53665500,
        56490000,
        105,
        'diskon 5% | addons: binding,covbahan,extrahal:10,hardbox',
        'pending',
        2,
        '2026-04-27 05:59:07',
        '2026-04-29 03:14:30'
    ),
    (
        9,
        'GRADUATION BBBB',
        'Graduation — Photo & Video',
        5328000,
        0,
        100,
        'addons: gbooth_add,g4r:7',
        'pending',
        2,
        '2026-04-29 03:25:06',
        '2026-04-29 03:25:06'
    ),
    (
        10,
        'TK Al Bardah',
        'À La Carte — Cetak Only',
        9108000,
        0,
        33,
        '',
        'pending',
        4,
        '2026-04-30 04:25:17',
        '2026-04-30 04:25:17'
    ),
    (
        11,
        'SMA 70',
        'Full Service — Handy Book A4+',
        53000000,
        0,
        100,
        'addons: binding,hardbox',
        'pending',
        2,
        '2026-05-08 10:20:18',
        '2026-05-08 10:20:18'
    ),
    (
        12,
        'test sekolah',
        'Full Service — Handy Book A4+',
        15000000,
        0,
        25,
        '',
        'pending',
        1,
        '2026-06-09 12:16:16',
        '2026-06-09 12:16:16'
    ),
    (
        13,
        'test b',
        'Full Service — Handy Book A4+',
        15000000,
        0,
        10,
        '',
        'pending',
        1,
        '2026-06-09 12:17:13',
        '2026-06-09 12:17:13'
    );

-- --------------------------------------------------------

--
-- Table structure for table `pricing_factors`
--

CREATE TABLE `pricing_factors` (
    `id` int NOT NULL,
    `category` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `factor_name` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `factor_value` decimal(10, 4) NOT NULL,
    `description` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `pricing_factors`
--

INSERT INTO
    `pricing_factors` (
        `id`,
        `category`,
        `factor_name`,
        `factor_value`,
        `description`,
        `created_at`,
        `updated_at`
    )
VALUES (
        15,
        'cetak',
        'handy',
        1.0000,
        '',
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        16,
        'cetak',
        'minimal',
        0.9500,
        '',
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        17,
        'cetak',
        'large',
        1.1500,
        '',
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        18,
        'alacarte',
        'ebook',
        0.7300,
        '',
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        19,
        'alacarte',
        'editcetak',
        0.6200,
        '',
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        20,
        'alacarte',
        'desain',
        0.2200,
        '',
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    ),
    (
        21,
        'alacarte',
        'cetakonly',
        0.3000,
        '',
        '2026-04-16 03:12:16',
        '2026-04-16 03:12:16'
    );

-- --------------------------------------------------------

--
-- Table structure for table `roles`
--

CREATE TABLE `roles` (
    `id` int NOT NULL,
    `name` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `label` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `permissions` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL,
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_general_ci;

--
-- Dumping data for table `roles`
--

INSERT INTO
    `roles` (
        `id`,
        `name`,
        `label`,
        `permissions`,
        `created_at`
    )
VALUES (
        1,
        'admin',
        'Administrator',
        '[\"create_user\",\"edit_user\",\"delete_user\",\"view_reports\",\"manage_settings\"]',
        '2026-04-16 03:12:16'
    ),
    (
        2,
        'manager',
        'Manager',
        '[\"create_penawaran\",\"edit_penawaran\",\"view_reports\"]',
        '2026-04-16 03:12:16'
    ),
    (
        3,
        'staff',
        'Staff',
        '[\"create_penawaran\",\"view_penawaran\"]',
        '2026-04-16 03:12:16'
    );

-- --------------------------------------------------------

--
-- Table structure for table `settings`
--

CREATE TABLE `settings` (
    `id` int NOT NULL,
    `setting_key` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `setting_value` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `updated_by` int DEFAULT NULL,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `settings`
--

INSERT INTO
    `settings` (
        `id`,
        `setting_key`,
        `setting_value`,
        `updated_by`,
        `updated_at`
    )
VALUES (
        1,
        'grad_packages',
        '[{\"id\":\"gphv\",\"name\":\"Photo & Video\",\"price\":4500000,\"desc\":\"2 Fotografer + 1 Videografer, 50 foto edited, video cinematic 2\\u20134 mnt, G-Drive, 4 jam coverage, transport jabodetabek\",\"color\":\"acc\"},{\"id\":\"gvideo\",\"name\":\"Video Only\",\"price\":2000000,\"desc\":\"1 Videografer, video cinematic 2\\u20135 mnt, G-Drive, 4 jam coverage, transport jabodetabek\",\"color\":\"\"},{\"id\":\"gphoto\",\"name\":\"Photo Only\",\"price\":2750000,\"desc\":\"2 Fotografer, 100 foto edited, G-Drive, 4 jam coverage, transport jabodetabek\",\"color\":\"\"},{\"id\":\"gbooth\",\"name\":\"Photo Booth\",\"price\":3850000,\"desc\":\"1\\u20132 Crew profesional, backdrop wisuda, lighting studio, Selfiebox Machine, unlimited print 4R, max 3 jam, softcopy + QR Code realtime, transport jabodetabek\",\"color\":\"\"},{\"id\":\"g360\",\"name\":\"Glamation 360\\u00b0\",\"price\":4100000,\"desc\":\"1\\u20132 Crew profesional, MP4, LCD 50in preview, GoPro\\/iPhone 12 Pro, overlay design free, max 3 jam, QR Code realtime, transport jabodetabek\",\"color\":\"\"},{\"id\":\"gcomplete\",\"name\":\"Complete Package\",\"price\":7750000,\"desc\":\"Photo (2 foto, 100 edited) + Video (1 videografer, cinematic 2\\u20134 mnt) + Photo Booth (unlimited print 4R, max 3 jam, QR Code), transport jabodetabek\",\"color\":\"feat\"}]',
        2,
        '2026-04-06 03:02:34'
    ),
    (
        3,
        'oh',
        '{\"total\":65540000,\"marketing\":12750000,\"creative\":7670000,\"designer\":16700000,\"pm\":7200000,\"sosmed\":6430000,\"freelance\":3204000,\"ops\":11586000}',
        NULL,
        '2026-04-06 04:27:45'
    ),
    (
        4,
        'cetak_f',
        '{\"handy\":1.0, \"minimal\":0.95, \"large\":1.15}',
        NULL,
        '2026-04-06 04:27:45'
    ),
    (
        5,
        'alc_f',
        '{\"ebook\":0.72, \"editcetak\":0.62, \"desain\":0.22, \"cetakonly\":0.30}',
        NULL,
        '2026-04-06 04:27:45'
    ),
    (
        6,
        'fs',
        '{\"handy\":[[30,50,465000,30],[51,75,415000,30],[76,100,370000,45],[101,125,350000,55],[126,150,335000,60],[151,175,315000,65],[176,200,295000,75],[201,225,260000,80],[226,250,250000,80],[251,275,240000,90],[276,300,230000,100],[300,325,220000,100],[326,350,210000,120],[351,375,200000,120],[376,400,190000,135],[401,425,185000,135],[426,450,165000,145],[451,475,175000,150],[476,500,150000,160]],\"minimal\":[[30,50,450000,30],[51,75,400000,30],[76,100,355000,45],[101,125,335000,55],[126,150,320000,60],[151,175,300000,65],[176,200,280000,75],[201,225,245000,80],[226,250,235000,80],[251,275,240000,90],[276,300,215000,100],[300,325,205000,100],[326,350,195000,120],[351,375,185000,120],[376,400,180000,135],[401,425,170000,135],[426,450,160000,145],[451,475,150000,150],[476,500,140000,160]],\"large\":[[30,50,480000,30],[51,75,430000,30],[76,100,405000,45],[101,125,365000,55],[126,150,350000,60],[151,175,330000,65],[176,200,310000,75],[201,225,275000,80],[226,250,265000,80],[251,275,255000,90],[276,300,245000,100],[300,325,235000,100],[326,350,225000,120],[351,375,215000,120],[376,400,205000,135],[401,425,195000,135],[426,450,175000,145],[451,475,165000,150],[476,500,155000,160]]}',
        NULL,
        '2026-04-06 04:27:45'
    ),
    (
        7,
        'addon_data',
        '{\"finishing\":[{\"id\":\"binding\",\"name\":\"Binding Paku/Jepang/Spiral\",\"type\":\"flat\",\"tiers\":[[25,75,50000],[76,150,35000],[151,9999,30000]]},{\"id\":\"popup\",\"name\":\"Pop Up 2D\",\"type\":\"flat\",\"tiers\":[[25,75,55000],[76,150,40000],[151,9999,35000]]},{\"id\":\"tunnel\",\"name\":\"Cover Tunnel\",\"type\":\"flat\",\"tiers\":[[25,75,75000],[76,150,60000],[151,9999,50000]]},{\"id\":\"klip\",\"name\":\"Cover Klip/Cetekan\",\"type\":\"flat\",\"tiers\":[[25,75,15000],[76,150,10000],[151,9999,8000]]},{\"id\":\"covbahan\",\"name\":\"Cover Bahan\",\"type\":\"flat\",\"tiers\":[[25,75,55000],[76,150,40000],[151,9999,35000]]}],\"kertas\":[{\"id\":\"ivory\",\"name\":\"Ivory Paper\",\"type\":\"per_hal\",\"tiers\":[[25,50,450],[51,100,250],[101,150,200],[151,9999,150]]},{\"id\":\"laminasi\",\"name\":\"Laminasi Paper\",\"type\":\"per_hal\",\"tiers\":[[25,50,600],[51,100,450],[101,150,400],[151,9999,350]]}],\"halaman\":[{\"id\":\"extrahal\",\"name\":\"Halaman Tambahan\",\"type\":\"extra_hal\",\"tiers\":[[25,50,3000],[51,100,2000],[101,150,1300],[151,9999,1000]]}],\"video\":[{\"id\":\"drone\",\"name\":\"Drone Video (1\\u20132 mnt)\",\"type\":\"flat_video\",\"price\":1500000},{\"id\":\"docudrama\",\"name\":\"Docudrama Video (5\\u201310 mnt)\",\"type\":\"flat_video\",\"price\":3000000}],\"pkg1\":[{\"id\":\"slidebox\",\"name\":\"Slide Box\",\"type\":\"flat\",\"tiers\":[[25,50,45000],[51,100,40000],[101,150,35000],[151,200,30000],[201,9999,25000]]},{\"id\":\"stdbox1\",\"name\":\"Standart Box 1\",\"type\":\"flat\",\"tiers\":[[25,50,150000],[51,100,95000],[101,150,80000],[151,200,70000],[201,9999,65000]]},{\"id\":\"stdbox2\",\"name\":\"Standart Box 2\",\"type\":\"flat\",\"tiers\":[[25,50,150000],[51,100,100000],[101,150,80000],[151,200,75000],[201,9999,70000]]},{\"id\":\"hardbox\",\"name\":\"Hard Box 3 (Akrilik)\",\"type\":\"flat\",\"tiers\":[[25,50,125000],[51,100,100000],[101,150,90000],[151,200,80000],[201,9999,75000]]}],\"pkg2\":[{\"id\":\"cbox1\",\"name\":\"Custom Box 1\",\"type\":\"flat\",\"tiers\":[[25,50,200000],[51,100,170000],[101,150,130000],[151,200,120000],[201,9999,110000]]},{\"id\":\"cbox2\",\"name\":\"Custom Box 2\",\"type\":\"flat\",\"tiers\":[[25,50,165000],[51,100,150000],[101,150,130000],[151,200,120000],[201,9999,110000]]},{\"id\":\"cbox3\",\"name\":\"Custom Box 3\",\"type\":\"flat\",\"tiers\":[[25,50,200000],[51,100,170000],[101,150,130000],[151,200,120000],[201,9999,110000]]},{\"id\":\"cbox4\",\"name\":\"Custom Box 4\",\"type\":\"flat\",\"tiers\":[[25,50,200000],[51,100,170000],[101,150,140000],[151,200,130000],[201,9999,120000]]},{\"id\":\"cbox5\",\"name\":\"Custom Box 5\",\"type\":\"flat\",\"tiers\":[[25,50,200000],[51,100,170000],[101,150,145000],[151,200,135000],[201,9999,130000]]}]}',
        NULL,
        '2026-04-06 04:27:45'
    ),
    (
        8,
        'cetak_base',
        '[{\"lo\":30,\"hi\":50,\"label\":\"30\\u201350 siswa\",\"pages\":{\"30\":92000,\"45\":102000,\"60\":115000,\"65\":127000,\"75\":140000,\"80\":140000,\"90\":152000,\"100\":165000,\"110\":176000,\"120\":176000,\"135\":176000,\"150\":176000,\"160\":176000}},{\"lo\":51,\"hi\":75,\"label\":\"51\\u201375 siswa\",\"pages\":{\"30\":80000,\"45\":90000,\"60\":100000,\"65\":110000,\"75\":122000,\"80\":122000,\"90\":134000,\"100\":145000,\"110\":158000,\"120\":162000,\"135\":165000,\"150\":168000,\"160\":170000}},{\"lo\":76,\"hi\":100,\"label\":\"76\\u2013100 siswa\",\"pages\":{\"30\":70000,\"45\":80000,\"60\":90000,\"65\":98000,\"75\":108000,\"80\":108000,\"90\":118000,\"100\":130000,\"110\":140000,\"120\":145000,\"135\":150000,\"150\":155000,\"160\":160000}},{\"lo\":101,\"hi\":125,\"label\":\"101\\u2013125 siswa\",\"pages\":{\"30\":62000,\"45\":72000,\"60\":82000,\"65\":88000,\"75\":97000,\"80\":97000,\"90\":106000,\"100\":116000,\"110\":126000,\"120\":130000,\"135\":135000,\"150\":140000,\"160\":145000}},{\"lo\":126,\"hi\":150,\"label\":\"126\\u2013150 siswa\",\"pages\":{\"30\":58000,\"45\":68000,\"60\":76000,\"65\":82000,\"75\":90000,\"80\":90000,\"90\":98000,\"100\":108000,\"110\":118000,\"120\":122000,\"135\":127000,\"150\":132000,\"160\":137000}},{\"lo\":151,\"hi\":175,\"label\":\"151\\u2013175 siswa\",\"pages\":{\"30\":54000,\"45\":63000,\"60\":71000,\"65\":76000,\"75\":84000,\"80\":84000,\"90\":91000,\"100\":100000,\"110\":109000,\"120\":113000,\"135\":118000,\"150\":123000,\"160\":128000}},{\"lo\":176,\"hi\":200,\"label\":\"176\\u2013200 siswa\",\"pages\":{\"30\":50000,\"45\":59000,\"60\":66000,\"65\":71000,\"75\":78000,\"80\":78000,\"90\":85000,\"100\":93000,\"110\":101000,\"120\":105000,\"135\":110000,\"150\":115000,\"160\":119000}},{\"lo\":201,\"hi\":225,\"label\":\"201\\u2013225 siswa\",\"pages\":{\"30\":47000,\"45\":55000,\"60\":62000,\"65\":66000,\"75\":73000,\"80\":73000,\"90\":79000,\"100\":87000,\"110\":95000,\"120\":98000,\"135\":103000,\"150\":107000,\"160\":111000}},{\"lo\":226,\"hi\":250,\"label\":\"226\\u2013250 siswa\",\"pages\":{\"30\":44000,\"45\":52000,\"60\":58000,\"65\":62000,\"75\":68000,\"80\":68000,\"90\":74000,\"100\":81000,\"110\":88000,\"120\":92000,\"135\":96000,\"150\":100000,\"160\":104000}},{\"lo\":251,\"hi\":275,\"label\":\"251\\u2013275 siswa\",\"pages\":{\"30\":41000,\"45\":49000,\"60\":55000,\"65\":58000,\"75\":64000,\"80\":64000,\"90\":70000,\"100\":76000,\"110\":83000,\"120\":86000,\"135\":90000,\"150\":94000,\"160\":98000}},{\"lo\":276,\"hi\":300,\"label\":\"276\\u2013300 siswa\",\"pages\":{\"30\":39000,\"45\":46000,\"60\":52000,\"65\":55000,\"75\":60000,\"80\":60000,\"90\":66000,\"100\":72000,\"110\":78000,\"120\":81000,\"135\":85000,\"150\":89000,\"160\":92000}},{\"lo\":301,\"hi\":325,\"label\":\"301\\u2013325 siswa\",\"pages\":{\"30\":37000,\"45\":44000,\"60\":49000,\"65\":52000,\"75\":57000,\"80\":57000,\"90\":62000,\"100\":68000,\"110\":74000,\"120\":77000,\"135\":80000,\"150\":84000,\"160\":87000}},{\"lo\":326,\"hi\":350,\"label\":\"326\\u2013350 siswa\",\"pages\":{\"30\":35000,\"45\":42000,\"60\":47000,\"65\":50000,\"75\":54000,\"80\":54000,\"90\":59000,\"100\":65000,\"110\":70000,\"120\":73000,\"135\":76000,\"150\":80000,\"160\":83000}},{\"lo\":351,\"hi\":375,\"label\":\"351\\u2013375 siswa\",\"pages\":{\"30\":33000,\"45\":40000,\"60\":45000,\"65\":47000,\"75\":52000,\"80\":52000,\"90\":56000,\"100\":62000,\"110\":67000,\"120\":70000,\"135\":73000,\"150\":76000,\"160\":79000}},{\"lo\":376,\"hi\":400,\"label\":\"376\\u2013400 siswa\",\"pages\":{\"30\":31000,\"45\":38000,\"60\":42000,\"65\":45000,\"75\":49000,\"80\":49000,\"90\":53000,\"100\":58000,\"110\":63000,\"120\":66000,\"135\":69000,\"150\":72000,\"160\":75000}},{\"lo\":401,\"hi\":425,\"label\":\"401\\u2013425 siswa\",\"pages\":{\"30\":30000,\"45\":36000,\"60\":40000,\"65\":42000,\"75\":46000,\"80\":46000,\"90\":50000,\"100\":55000,\"110\":60000,\"120\":62000,\"135\":65000,\"150\":68000,\"160\":71000}},{\"lo\":426,\"hi\":450,\"label\":\"426\\u2013450 siswa\",\"pages\":{\"30\":28000,\"45\":34000,\"60\":38000,\"65\":40000,\"75\":44000,\"80\":44000,\"90\":48000,\"100\":52000,\"110\":57000,\"120\":59000,\"135\":62000,\"150\":65000,\"160\":67000}},{\"lo\":451,\"hi\":475,\"label\":\"451\\u2013475 siswa\",\"pages\":{\"30\":27000,\"45\":32000,\"60\":36000,\"65\":38000,\"75\":42000,\"80\":42000,\"90\":45000,\"100\":50000,\"110\":54000,\"120\":56000,\"135\":59000,\"150\":62000,\"160\":64000}},{\"lo\":476,\"hi\":500,\"label\":\"476\\u2013500 siswa\",\"pages\":{\"30\":26000,\"45\":31000,\"60\":34000,\"65\":36000,\"75\":40000,\"80\":40000,\"90\":43000,\"100\":47000,\"110\":51000,\"120\":53000,\"135\":56000,\"150\":59000,\"160\":61000}}]',
        NULL,
        '2026-04-06 04:27:45'
    ),
    (
        9,
        'alc_cfg',
        '{\"fs-handy\":{\"label\":\"Full Service \\u2014 Handy Book A4+\",\"fs\":true,\"pkg\":\"handy\",\"bySiswa\":true},\"fs-minimal\":{\"label\":\"Full Service \\u2014 Minimal Book SQ\",\"fs\":true,\"pkg\":\"minimal\",\"bySiswa\":true},\"fs-large\":{\"label\":\"Full Service \\u2014 Large Book B4\",\"fs\":true,\"pkg\":\"large\",\"bySiswa\":true},\"ac-ebook\":{\"label\":\"E-Book Package\",\"bySiswa\":true,\"factor\":\"ebook\"},\"ac-editcetak\":{\"label\":\"Edit+Desain+Cetak\",\"bySiswa\":true,\"factor\":\"editcetak\"},\"ac-fotohalf\":{\"label\":\"Foto Only (\\u00bd hari)\",\"bySiswa\":false,\"flat\":[3500000,5000000]},\"ac-fotofull\":{\"label\":\"Foto Only (full day)\",\"bySiswa\":false,\"flat\":[6000000,9000000]},\"ac-videod\":{\"label\":\"Drone Video\",\"bySiswa\":false,\"flat\":[1500000,1500000]},\"ac-videodoc\":{\"label\":\"Docudrama Video\",\"bySiswa\":false,\"flat\":[3000000,3000000]},\"ac-desain\":{\"label\":\"Desain Only\",\"bySiswa\":true,\"factor\":\"desain\",\"minPerBuku\":50000},\"ac-cetakonly\":{\"label\":\"Cetak Only\",\"bySiswa\":true,\"factor\":\"cetakonly\",\"minPerBuku\":30000}}',
        NULL,
        '2026-04-06 04:27:45'
    ),
    (
        10,
        'pdf_penutup',
        'Demikian penawaran yang kami sampaikan. Besar harapan kami untuk dapat berpartisipasi dalam project Anda. Hal–hal yang belum termasuk dan diatur di sini akan dibicarakan di kemudian hari apabila penawaran ini disetujui. Atas perhatian dan kerjasamanya kami sampaikan terima kasih.',
        NULL,
        '2026-04-29 10:33:44'
    );

-- --------------------------------------------------------

--
-- Table structure for table `spesifikasi_produk`
--

CREATE TABLE `spesifikasi_produk` (
    `id` int NOT NULL,
    `package_type` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `kategori` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'all',
    `label` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `value` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `display_order` int DEFAULT '0',
    `active` tinyint DEFAULT '1',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `spesifikasi_produk`
--

INSERT INTO
    `spesifikasi_produk` (
        `id`,
        `package_type`,
        `kategori`,
        `label`,
        `value`,
        `display_order`,
        `active`,
        `created_at`
    )
VALUES (
        1,
        'fullservice',
        'fs-handy',
        'Ukuran Buku',
        'A4+ (22 × 30 cm)',
        1,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        2,
        'fullservice',
        'fs-handy',
        'Jenis Kertas',
        'Matte Paper 150gsm',
        0,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        3,
        'fullservice',
        'fs-handy',
        'Cover',
        'Hard Cover, AC 190gsm, Laminasi Doff',
        3,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        4,
        'fullservice',
        'fs-handy',
        'Finishing',
        'Binding Jahit',
        4,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        6,
        'fullservice',
        'fs-minimal',
        'Ukuran Buku',
        'SQ (25 × 25 cm)',
        1,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        7,
        'fullservice',
        'fs-minimal',
        'Jenis Kertas',
        'Art Paper 120gsm',
        2,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        8,
        'fullservice',
        'fs-minimal',
        'Cover',
        'Soft Cover, AC 150gsm, Laminasi Doff',
        3,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        9,
        'fullservice',
        'fs-minimal',
        'Finishing',
        'Binding Jahit / Staples',
        4,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        10,
        'fullservice',
        'fs-minimal',
        'Packaging',
        'Plastik Wrap',
        5,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        11,
        'fullservice',
        'fs-large',
        'Ukuran Buku',
        'B4 (25 × 35 cm)',
        1,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        12,
        'fullservice',
        'fs-large',
        'Jenis Kertas',
        'Art Paper 170gsm',
        2,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        13,
        'fullservice',
        'fs-large',
        'Cover',
        'Hard Cover, AC 210gsm, Laminasi Doff',
        3,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        14,
        'fullservice',
        'fs-large',
        'Finishing',
        'Binding Jahit',
        4,
        1,
        '2026-04-28 09:09:32'
    ),
    (
        15,
        'fullservice',
        'fs-large',
        'Packaging',
        'Slongsong',
        5,
        1,
        '2026-04-28 09:09:32'
    );

-- --------------------------------------------------------

--
-- Table structure for table `syarat_ketentuan`
--

CREATE TABLE `syarat_ketentuan` (
    `id` int NOT NULL,
    `package_type` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `kategori` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'all',
    `label` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `detail` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `display_order` int DEFAULT '0',
    `active` tinyint(1) DEFAULT '1',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `syarat_ketentuan`
--

INSERT INTO
    `syarat_ketentuan` (
        `id`,
        `package_type`,
        `kategori`,
        `label`,
        `detail`,
        `display_order`,
        `active`,
        `created_at`
    )
VALUES (
        1,
        'fullservice',
        'all',
        'Harga berlaku untuk minimal {siswa} pemesan Buku Tahunan.',
        '',
        1,
        1,
        '2026-04-28 07:59:16'
    ),
    (
        2,
        'fullservice',
        'all',
        'Harga bersifat penawaran dan dapat berubah sesuai kesepakatan.',
        NULL,
        2,
        1,
        '2026-04-28 07:59:16'
    ),
    (
        3,
        'graduation',
        'all',
        'Harga berlaku untuk event yang telah disepakati.',
        NULL,
        1,
        1,
        '2026-04-28 07:59:16'
    ),
    (
        4,
        'graduation',
        'all',
        'Harga bersifat penawaran dan dapat berubah sesuai kesepakatan.',
        NULL,
        2,
        1,
        '2026-04-28 07:59:16'
    ),
    (
        5,
        'alacarte',
        'all',
        'Harga berlaku sesuai spesifikasi yang tercantum.',
        NULL,
        1,
        1,
        '2026-04-28 07:59:16'
    ),
    (
        6,
        'alacarte',
        'all',
        'Harga bersifat penawaran dan dapat berubah sesuai kesepakatan.',
        NULL,
        2,
        1,
        '2026-04-28 07:59:16'
    ),
    (
        8,
        'fullservice',
        'all',
        'Harga di atas masih bersifat tentative (dapat berubah) sesuai kesepakatan nantinya.',
        NULL,
        10,
        1,
        '2026-04-28 07:59:16'
    ),
    (
        9,
        'graduation',
        'all',
        'Harga di atas masih bersifat tentative (dapat berubah) sesuai kesepakatan nantinya.',
        NULL,
        10,
        1,
        '2026-04-28 07:59:16'
    ),
    (
        10,
        'alacarte',
        'all',
        'Harga di atas masih bersifat tentative (dapat berubah) sesuai kesepakatan nantinya.',
        NULL,
        10,
        1,
        '2026-04-28 07:59:16'
    );

-- --------------------------------------------------------

--
-- Table structure for table `tbl_addons`
--

CREATE TABLE `tbl_addons` (
    `id` int NOT NULL,
    `category` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `sub_id` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `name` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `type` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `price` int DEFAULT '0',
    `min_qty` int DEFAULT '0',
    `max_qty` int DEFAULT '9999'
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `tbl_addons`
--

INSERT INTO
    `tbl_addons` (
        `id`,
        `category`,
        `sub_id`,
        `name`,
        `type`,
        `price`,
        `min_qty`,
        `max_qty`
    )
VALUES (
        3674,
        'finishing',
        'binding',
        'Binding Paku/Jepang/Spiral',
        'flat',
        95000,
        10,
        19
    ),
    (
        3675,
        'finishing',
        'binding',
        'Binding Paku/Jepang/Spiral',
        'flat',
        75000,
        20,
        35
    ),
    (
        3676,
        'finishing',
        'binding',
        'Binding Paku/Jepang/Spiral',
        'flat',
        65000,
        36,
        50
    ),
    (
        3677,
        'finishing',
        'binding',
        'Binding Paku/Jepang/Spiral',
        'flat',
        50000,
        51,
        75
    ),
    (
        3678,
        'finishing',
        'binding',
        'Binding Paku/Jepang/Spiral',
        'flat',
        45000,
        76,
        100
    ),
    (
        3679,
        'finishing',
        'binding',
        'Binding Paku/Jepang/Spiral',
        'flat',
        40000,
        101,
        150
    ),
    (
        3680,
        'finishing',
        'binding',
        'Binding Paku/Jepang/Spiral',
        'flat',
        35000,
        151,
        600
    ),
    (
        3681,
        'finishing',
        'covbahan',
        'Cover Bahan',
        'flat',
        75000,
        10,
        24
    ),
    (
        3682,
        'finishing',
        'covbahan',
        'Cover Bahan',
        'flat',
        55000,
        25,
        75
    ),
    (
        3683,
        'finishing',
        'covbahan',
        'Cover Bahan',
        'flat',
        40000,
        76,
        150
    ),
    (
        3684,
        'finishing',
        'covbahan',
        'Cover Bahan',
        'flat',
        35000,
        151,
        9999
    ),
    (
        3685,
        'finishing',
        'klip',
        'Cover Klip/Cetekan',
        'flat',
        35000,
        10,
        24
    ),
    (
        3686,
        'finishing',
        'klip',
        'Cover Klip/Cetekan',
        'flat',
        15000,
        25,
        75
    ),
    (
        3687,
        'finishing',
        'klip',
        'Cover Klip/Cetekan',
        'flat',
        10000,
        76,
        150
    ),
    (
        3688,
        'finishing',
        'klip',
        'Cover Klip/Cetekan',
        'flat',
        8000,
        151,
        9999
    ),
    (
        3689,
        'finishing',
        'popup',
        'Pop Up 2D',
        'flat',
        95000,
        10,
        19
    ),
    (
        3690,
        'finishing',
        'popup',
        'Pop Up 2D',
        'flat',
        75000,
        20,
        35
    ),
    (
        3691,
        'finishing',
        'popup',
        'Pop Up 2D',
        'flat',
        65000,
        36,
        50
    ),
    (
        3692,
        'finishing',
        'popup',
        'Pop Up 2D',
        'flat',
        50000,
        51,
        75
    ),
    (
        3693,
        'finishing',
        'popup',
        'Pop Up 2D',
        'flat',
        45000,
        76,
        100
    ),
    (
        3694,
        'finishing',
        'popup',
        'Pop Up 2D',
        'flat',
        40000,
        101,
        150
    ),
    (
        3695,
        'finishing',
        'popup',
        'Pop Up 2D',
        'flat',
        35000,
        151,
        600
    ),
    (
        3696,
        'finishing',
        'tunnel',
        'Cover Tunnel',
        'flat',
        105000,
        10,
        19
    ),
    (
        3697,
        'finishing',
        'tunnel',
        'Cover Tunnel',
        'flat',
        85000,
        20,
        35
    ),
    (
        3698,
        'finishing',
        'tunnel',
        'Cover Tunnel',
        'flat',
        75000,
        36,
        50
    ),
    (
        3699,
        'finishing',
        'tunnel',
        'Cover Tunnel',
        'flat',
        65000,
        51,
        75
    ),
    (
        3700,
        'finishing',
        'tunnel',
        'Cover Tunnel',
        'flat',
        60000,
        76,
        100
    ),
    (
        3701,
        'finishing',
        'tunnel',
        'Cover Tunnel',
        'flat',
        55000,
        101,
        150
    ),
    (
        3702,
        'finishing',
        'tunnel',
        'Cover Tunnel',
        'flat',
        50000,
        151,
        600
    ),
    (
        3703,
        'kertas',
        'ivory',
        'Ivory Paper',
        'flat',
        650,
        10,
        24
    ),
    (
        3704,
        'kertas',
        'ivory',
        'Ivory Paper',
        'flat',
        450,
        25,
        50
    ),
    (
        3705,
        'kertas',
        'ivory',
        'Ivory Paper',
        'flat',
        250,
        51,
        100
    ),
    (
        3706,
        'kertas',
        'ivory',
        'Ivory Paper',
        'flat',
        200,
        101,
        150
    ),
    (
        3707,
        'kertas',
        'ivory',
        'Ivory Paper',
        'flat',
        150,
        151,
        9999
    ),
    (
        3708,
        'kertas',
        'laminasi',
        'Laminasi Paper',
        'flat',
        800,
        10,
        24
    ),
    (
        3709,
        'kertas',
        'laminasi',
        'Laminasi Paper',
        'flat',
        600,
        25,
        50
    ),
    (
        3710,
        'kertas',
        'laminasi',
        'Laminasi Paper',
        'flat',
        450,
        51,
        100
    ),
    (
        3711,
        'kertas',
        'laminasi',
        'Laminasi Paper',
        'flat',
        400,
        101,
        150
    ),
    (
        3712,
        'kertas',
        'laminasi',
        'Laminasi Paper',
        'flat',
        350,
        151,
        9999
    ),
    (
        3713,
        'halaman',
        'extrahal',
        'Halaman Tambahan',
        'flat',
        7000,
        10,
        19
    ),
    (
        3714,
        'halaman',
        'extrahal',
        'Halaman Tambahan',
        'flat',
        5000,
        20,
        30
    ),
    (
        3715,
        'halaman',
        'extrahal',
        'Halaman Tambahan',
        'flat',
        3000,
        31,
        50
    ),
    (
        3716,
        'halaman',
        'extrahal',
        'Halaman Tambahan',
        'flat',
        2000,
        51,
        75
    ),
    (
        3717,
        'halaman',
        'extrahal',
        'Halaman Tambahan',
        'flat',
        1500,
        76,
        100
    ),
    (
        3718,
        'halaman',
        'extrahal',
        'Halaman Tambahan',
        'flat',
        1300,
        101,
        150
    ),
    (
        3719,
        'halaman',
        'extrahal',
        'Halaman Tambahan',
        'flat',
        1000,
        151,
        600
    ),
    (
        3720,
        'video',
        'docudrama',
        'Docudrama Video (5–10 mnt)',
        'flat_video',
        3500000,
        0,
        9999
    ),
    (
        3721,
        'video',
        'drone',
        'Drone Video (1–2 mnt)',
        'flat_video',
        2000000,
        0,
        9999
    ),
    (
        3722,
        'pkg1',
        'hardbox',
        'Hard Box 3 (Akrilik)',
        'flat',
        200000,
        10,
        19
    ),
    (
        3723,
        'pkg1',
        'hardbox',
        'Hard Box 3 (Akrilik)',
        'flat',
        150000,
        20,
        30
    ),
    (
        3724,
        'pkg1',
        'hardbox',
        'Hard Box 3 (Akrilik)',
        'flat',
        125000,
        31,
        50
    ),
    (
        3725,
        'pkg1',
        'hardbox',
        'Hard Box 3 (Akrilik)',
        'flat',
        110000,
        51,
        75
    ),
    (
        3726,
        'pkg1',
        'hardbox',
        'Hard Box 3 (Akrilik)',
        'flat',
        100000,
        76,
        100
    ),
    (
        3727,
        'pkg1',
        'hardbox',
        'Hard Box 3 (Akrilik)',
        'flat',
        90000,
        101,
        150
    ),
    (
        3728,
        'pkg1',
        'hardbox',
        'Hard Box 3 (Akrilik)',
        'flat',
        80000,
        151,
        600
    ),
    (
        3729,
        'pkg1',
        'slidebox',
        'Slide Box',
        'flat',
        75000,
        10,
        24
    ),
    (
        3730,
        'pkg1',
        'slidebox',
        'Slide Box',
        'flat',
        45000,
        25,
        50
    ),
    (
        3731,
        'pkg1',
        'slidebox',
        'Slide Box',
        'flat',
        40000,
        51,
        100
    ),
    (
        3732,
        'pkg1',
        'slidebox',
        'Slide Box',
        'flat',
        35000,
        101,
        150
    ),
    (
        3733,
        'pkg1',
        'slidebox',
        'Slide Box',
        'flat',
        30000,
        151,
        200
    ),
    (
        3734,
        'pkg1',
        'slidebox',
        'Slide Box',
        'flat',
        25000,
        201,
        9999
    ),
    (
        3735,
        'pkg1',
        'stdbox1',
        'Standart Box 1',
        'flat',
        200000,
        10,
        24
    ),
    (
        3736,
        'pkg1',
        'stdbox1',
        'Standart Box 1',
        'flat',
        150000,
        25,
        50
    ),
    (
        3737,
        'pkg1',
        'stdbox1',
        'Standart Box 1',
        'flat',
        95000,
        51,
        100
    ),
    (
        3738,
        'pkg1',
        'stdbox1',
        'Standart Box 1',
        'flat',
        80000,
        101,
        150
    ),
    (
        3739,
        'pkg1',
        'stdbox1',
        'Standart Box 1',
        'flat',
        70000,
        151,
        200
    ),
    (
        3740,
        'pkg1',
        'stdbox1',
        'Standart Box 1',
        'flat',
        65000,
        201,
        9999
    ),
    (
        3741,
        'pkg1',
        'stdbox2',
        'Standart Box 2',
        'flat',
        200000,
        10,
        24
    ),
    (
        3742,
        'pkg1',
        'stdbox2',
        'Standart Box 2',
        'flat',
        150000,
        25,
        50
    ),
    (
        3743,
        'pkg1',
        'stdbox2',
        'Standart Box 2',
        'flat',
        100000,
        51,
        100
    ),
    (
        3744,
        'pkg1',
        'stdbox2',
        'Standart Box 2',
        'flat',
        80000,
        101,
        150
    ),
    (
        3745,
        'pkg1',
        'stdbox2',
        'Standart Box 2',
        'flat',
        75000,
        151,
        200
    ),
    (
        3746,
        'pkg1',
        'stdbox2',
        'Standart Box 2',
        'flat',
        70000,
        201,
        9999
    ),
    (
        3747,
        'pkg2',
        'cbox1',
        'Custom Box 1',
        'flat',
        250000,
        10,
        24
    ),
    (
        3748,
        'pkg2',
        'cbox1',
        'Custom Box 1',
        'flat',
        200000,
        25,
        50
    ),
    (
        3749,
        'pkg2',
        'cbox1',
        'Custom Box 1',
        'flat',
        170000,
        51,
        100
    ),
    (
        3750,
        'pkg2',
        'cbox1',
        'Custom Box 1',
        'flat',
        130000,
        101,
        150
    ),
    (
        3751,
        'pkg2',
        'cbox1',
        'Custom Box 1',
        'flat',
        120000,
        151,
        200
    ),
    (
        3752,
        'pkg2',
        'cbox1',
        'Custom Box 1',
        'flat',
        110000,
        201,
        9999
    ),
    (
        3753,
        'pkg2',
        'cbox2',
        'Custom Box 2',
        'flat',
        215000,
        10,
        24
    ),
    (
        3754,
        'pkg2',
        'cbox2',
        'Custom Box 2',
        'flat',
        165000,
        25,
        50
    ),
    (
        3755,
        'pkg2',
        'cbox2',
        'Custom Box 2',
        'flat',
        150000,
        51,
        100
    ),
    (
        3756,
        'pkg2',
        'cbox2',
        'Custom Box 2',
        'flat',
        130000,
        101,
        150
    ),
    (
        3757,
        'pkg2',
        'cbox2',
        'Custom Box 2',
        'flat',
        120000,
        151,
        200
    ),
    (
        3758,
        'pkg2',
        'cbox2',
        'Custom Box 2',
        'flat',
        110000,
        201,
        9999
    ),
    (
        3759,
        'pkg2',
        'cbox3',
        'Custom Box 3',
        'flat',
        250000,
        10,
        24
    ),
    (
        3760,
        'pkg2',
        'cbox3',
        'Custom Box 3',
        'flat',
        200000,
        25,
        50
    ),
    (
        3761,
        'pkg2',
        'cbox3',
        'Custom Box 3',
        'flat',
        170000,
        51,
        100
    ),
    (
        3762,
        'pkg2',
        'cbox3',
        'Custom Box 3',
        'flat',
        130000,
        101,
        150
    ),
    (
        3763,
        'pkg2',
        'cbox3',
        'Custom Box 3',
        'flat',
        120000,
        151,
        200
    ),
    (
        3764,
        'pkg2',
        'cbox3',
        'Custom Box 3',
        'flat',
        110000,
        201,
        9999
    ),
    (
        3765,
        'pkg2',
        'cbox4',
        'Custom Box 4',
        'flat',
        250000,
        10,
        19
    ),
    (
        3766,
        'pkg2',
        'cbox4',
        'Custom Box 4',
        'flat',
        200000,
        25,
        50
    ),
    (
        3767,
        'pkg2',
        'cbox4',
        'Custom Box 4',
        'flat',
        170000,
        51,
        100
    ),
    (
        3768,
        'pkg2',
        'cbox4',
        'Custom Box 4',
        'flat',
        140000,
        101,
        150
    ),
    (
        3769,
        'pkg2',
        'cbox4',
        'Custom Box 4',
        'flat',
        130000,
        151,
        200
    ),
    (
        3770,
        'pkg2',
        'cbox4',
        'Custom Box 4',
        'flat',
        120000,
        201,
        9999
    ),
    (
        3771,
        'pkg2',
        'cbox5',
        'Custom Box 5',
        'flat',
        250000,
        10,
        19
    ),
    (
        3772,
        'pkg2',
        'cbox5',
        'Custom Box 5',
        'flat',
        200000,
        25,
        50
    ),
    (
        3773,
        'pkg2',
        'cbox5',
        'Custom Box 5',
        'flat',
        170000,
        51,
        100
    ),
    (
        3774,
        'pkg2',
        'cbox5',
        'Custom Box 5',
        'flat',
        145000,
        101,
        150
    ),
    (
        3775,
        'pkg2',
        'cbox5',
        'Custom Box 5',
        'flat',
        135000,
        151,
        200
    ),
    (
        3776,
        'pkg2',
        'cbox5',
        'Custom Box 5',
        'flat',
        130000,
        201,
        9999
    );

-- --------------------------------------------------------

--
-- Table structure for table `tbl_cetak_prices`
--

CREATE TABLE `tbl_cetak_prices` (
    `id` int NOT NULL,
    `range_idx` int DEFAULT NULL,
    `label` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `min_siswa` int DEFAULT NULL,
    `max_siswa` int DEFAULT NULL,
    `pages` int DEFAULT NULL,
    `harga` int DEFAULT NULL
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `tbl_cetak_prices`
--

INSERT INTO
    `tbl_cetak_prices` (
        `id`,
        `range_idx`,
        `label`,
        `min_siswa`,
        `max_siswa`,
        `pages`,
        `harga`
    )
VALUES (
        1,
        0,
        '30-50 siswa',
        30,
        50,
        30,
        92000
    ),
    (
        2,
        0,
        '30-50 siswa',
        30,
        50,
        45,
        102000
    ),
    (
        3,
        0,
        '30-50 siswa',
        30,
        50,
        60,
        115000
    ),
    (
        4,
        0,
        '30-50 siswa',
        30,
        50,
        65,
        127000
    ),
    (
        5,
        0,
        '30-50 siswa',
        30,
        50,
        75,
        140000
    ),
    (
        6,
        0,
        '30-50 siswa',
        30,
        50,
        80,
        140000
    ),
    (
        7,
        0,
        '30-50 siswa',
        30,
        50,
        90,
        152000
    ),
    (
        8,
        0,
        '30-50 siswa',
        30,
        50,
        100,
        165000
    ),
    (
        9,
        0,
        '30-50 siswa',
        30,
        50,
        110,
        176000
    ),
    (
        10,
        0,
        '30-50 siswa',
        30,
        50,
        120,
        176000
    ),
    (
        11,
        0,
        '30-50 siswa',
        30,
        50,
        135,
        176000
    ),
    (
        12,
        0,
        '30-50 siswa',
        30,
        50,
        150,
        176000
    ),
    (
        13,
        0,
        '30-50 siswa',
        30,
        50,
        160,
        176000
    ),
    (
        14,
        1,
        '51-75 siswa',
        51,
        75,
        30,
        80000
    ),
    (
        15,
        1,
        '51-75 siswa',
        51,
        75,
        45,
        90000
    ),
    (
        16,
        1,
        '51-75 siswa',
        51,
        75,
        60,
        100000
    ),
    (
        17,
        1,
        '51-75 siswa',
        51,
        75,
        65,
        110000
    ),
    (
        18,
        1,
        '51-75 siswa',
        51,
        75,
        75,
        122000
    ),
    (
        19,
        1,
        '51-75 siswa',
        51,
        75,
        80,
        122000
    ),
    (
        20,
        1,
        '51-75 siswa',
        51,
        75,
        90,
        134000
    ),
    (
        21,
        1,
        '51-75 siswa',
        51,
        75,
        100,
        145000
    ),
    (
        22,
        1,
        '51-75 siswa',
        51,
        75,
        110,
        158000
    ),
    (
        23,
        1,
        '51-75 siswa',
        51,
        75,
        120,
        162000
    ),
    (
        24,
        1,
        '51-75 siswa',
        51,
        75,
        135,
        165000
    ),
    (
        25,
        1,
        '51-75 siswa',
        51,
        75,
        150,
        168000
    ),
    (
        26,
        1,
        '51-75 siswa',
        51,
        75,
        160,
        170000
    ),
    (
        27,
        2,
        '76-100 siswa',
        76,
        100,
        30,
        70000
    ),
    (
        28,
        2,
        '76-100 siswa',
        76,
        100,
        45,
        80000
    ),
    (
        29,
        2,
        '76-100 siswa',
        76,
        100,
        60,
        90000
    ),
    (
        30,
        2,
        '76-100 siswa',
        76,
        100,
        65,
        98000
    ),
    (
        31,
        2,
        '76-100 siswa',
        76,
        100,
        75,
        108000
    ),
    (
        32,
        2,
        '76-100 siswa',
        76,
        100,
        80,
        108000
    ),
    (
        33,
        2,
        '76-100 siswa',
        76,
        100,
        90,
        118000
    ),
    (
        34,
        2,
        '76-100 siswa',
        76,
        100,
        100,
        130000
    ),
    (
        35,
        2,
        '76-100 siswa',
        76,
        100,
        110,
        140000
    ),
    (
        36,
        2,
        '76-100 siswa',
        76,
        100,
        120,
        145000
    ),
    (
        37,
        2,
        '76-100 siswa',
        76,
        100,
        135,
        150000
    ),
    (
        38,
        2,
        '76-100 siswa',
        76,
        100,
        150,
        155000
    ),
    (
        39,
        2,
        '76-100 siswa',
        76,
        100,
        160,
        160000
    ),
    (
        40,
        3,
        '101-125 siswa',
        101,
        125,
        30,
        62000
    ),
    (
        41,
        3,
        '101-125 siswa',
        101,
        125,
        45,
        72000
    ),
    (
        42,
        3,
        '101-125 siswa',
        101,
        125,
        60,
        82000
    ),
    (
        43,
        3,
        '101-125 siswa',
        101,
        125,
        65,
        88000
    ),
    (
        44,
        3,
        '101-125 siswa',
        101,
        125,
        75,
        97000
    ),
    (
        45,
        3,
        '101-125 siswa',
        101,
        125,
        80,
        97000
    ),
    (
        46,
        3,
        '101-125 siswa',
        101,
        125,
        90,
        106000
    ),
    (
        47,
        3,
        '101-125 siswa',
        101,
        125,
        100,
        116000
    ),
    (
        48,
        3,
        '101-125 siswa',
        101,
        125,
        110,
        126000
    ),
    (
        49,
        3,
        '101-125 siswa',
        101,
        125,
        120,
        130000
    ),
    (
        50,
        3,
        '101-125 siswa',
        101,
        125,
        135,
        135000
    ),
    (
        51,
        3,
        '101-125 siswa',
        101,
        125,
        150,
        140000
    ),
    (
        52,
        3,
        '101-125 siswa',
        101,
        125,
        160,
        145000
    ),
    (
        53,
        4,
        '126-150 siswa',
        126,
        150,
        30,
        58000
    ),
    (
        54,
        4,
        '126-150 siswa',
        126,
        150,
        45,
        68000
    ),
    (
        55,
        4,
        '126-150 siswa',
        126,
        150,
        60,
        76000
    ),
    (
        56,
        4,
        '126-150 siswa',
        126,
        150,
        65,
        82000
    ),
    (
        57,
        4,
        '126-150 siswa',
        126,
        150,
        75,
        90000
    ),
    (
        58,
        4,
        '126-150 siswa',
        126,
        150,
        80,
        90000
    ),
    (
        59,
        4,
        '126-150 siswa',
        126,
        150,
        90,
        98000
    ),
    (
        60,
        4,
        '126-150 siswa',
        126,
        150,
        100,
        108000
    ),
    (
        61,
        4,
        '126-150 siswa',
        126,
        150,
        110,
        118000
    ),
    (
        62,
        4,
        '126-150 siswa',
        126,
        150,
        120,
        122000
    ),
    (
        63,
        4,
        '126-150 siswa',
        126,
        150,
        135,
        127000
    ),
    (
        64,
        4,
        '126-150 siswa',
        126,
        150,
        150,
        132000
    ),
    (
        65,
        4,
        '126-150 siswa',
        126,
        150,
        160,
        137000
    ),
    (
        66,
        5,
        '151-175 siswa',
        151,
        175,
        30,
        54000
    ),
    (
        67,
        5,
        '151-175 siswa',
        151,
        175,
        45,
        63000
    ),
    (
        68,
        5,
        '151-175 siswa',
        151,
        175,
        60,
        71000
    ),
    (
        69,
        5,
        '151-175 siswa',
        151,
        175,
        65,
        76000
    ),
    (
        70,
        5,
        '151-175 siswa',
        151,
        175,
        75,
        84000
    ),
    (
        71,
        5,
        '151-175 siswa',
        151,
        175,
        80,
        84000
    ),
    (
        72,
        5,
        '151-175 siswa',
        151,
        175,
        90,
        91000
    ),
    (
        73,
        5,
        '151-175 siswa',
        151,
        175,
        100,
        100000
    ),
    (
        74,
        5,
        '151-175 siswa',
        151,
        175,
        110,
        109000
    ),
    (
        75,
        5,
        '151-175 siswa',
        151,
        175,
        120,
        113000
    ),
    (
        76,
        5,
        '151-175 siswa',
        151,
        175,
        135,
        118000
    ),
    (
        77,
        5,
        '151-175 siswa',
        151,
        175,
        150,
        123000
    ),
    (
        78,
        5,
        '151-175 siswa',
        151,
        175,
        160,
        128000
    ),
    (
        79,
        6,
        '176-200 siswa',
        176,
        200,
        30,
        50000
    ),
    (
        80,
        6,
        '176-200 siswa',
        176,
        200,
        45,
        59000
    ),
    (
        81,
        6,
        '176-200 siswa',
        176,
        200,
        60,
        66000
    ),
    (
        82,
        6,
        '176-200 siswa',
        176,
        200,
        65,
        71000
    ),
    (
        83,
        6,
        '176-200 siswa',
        176,
        200,
        75,
        78000
    ),
    (
        84,
        6,
        '176-200 siswa',
        176,
        200,
        80,
        78000
    ),
    (
        85,
        6,
        '176-200 siswa',
        176,
        200,
        90,
        85000
    ),
    (
        86,
        6,
        '176-200 siswa',
        176,
        200,
        100,
        93000
    ),
    (
        87,
        6,
        '176-200 siswa',
        176,
        200,
        110,
        101000
    ),
    (
        88,
        6,
        '176-200 siswa',
        176,
        200,
        120,
        105000
    ),
    (
        89,
        6,
        '176-200 siswa',
        176,
        200,
        135,
        110000
    ),
    (
        90,
        6,
        '176-200 siswa',
        176,
        200,
        150,
        115000
    ),
    (
        91,
        6,
        '176-200 siswa',
        176,
        200,
        160,
        119000
    ),
    (
        92,
        7,
        '201-225 siswa',
        201,
        225,
        30,
        47000
    ),
    (
        93,
        7,
        '201-225 siswa',
        201,
        225,
        45,
        55000
    ),
    (
        94,
        7,
        '201-225 siswa',
        201,
        225,
        60,
        62000
    ),
    (
        95,
        7,
        '201-225 siswa',
        201,
        225,
        65,
        66000
    ),
    (
        96,
        7,
        '201-225 siswa',
        201,
        225,
        75,
        73000
    ),
    (
        97,
        7,
        '201-225 siswa',
        201,
        225,
        80,
        73000
    ),
    (
        98,
        7,
        '201-225 siswa',
        201,
        225,
        90,
        79000
    ),
    (
        99,
        7,
        '201-225 siswa',
        201,
        225,
        100,
        87000
    ),
    (
        100,
        7,
        '201-225 siswa',
        201,
        225,
        110,
        95000
    ),
    (
        101,
        7,
        '201-225 siswa',
        201,
        225,
        120,
        98000
    ),
    (
        102,
        7,
        '201-225 siswa',
        201,
        225,
        135,
        103000
    ),
    (
        103,
        7,
        '201-225 siswa',
        201,
        225,
        150,
        107000
    ),
    (
        104,
        7,
        '201-225 siswa',
        201,
        225,
        160,
        111000
    ),
    (
        105,
        8,
        '226-250 siswa',
        226,
        250,
        30,
        44000
    ),
    (
        106,
        8,
        '226-250 siswa',
        226,
        250,
        45,
        52000
    ),
    (
        107,
        8,
        '226-250 siswa',
        226,
        250,
        60,
        58000
    ),
    (
        108,
        8,
        '226-250 siswa',
        226,
        250,
        65,
        62000
    ),
    (
        109,
        8,
        '226-250 siswa',
        226,
        250,
        75,
        68000
    ),
    (
        110,
        8,
        '226-250 siswa',
        226,
        250,
        80,
        68000
    ),
    (
        111,
        8,
        '226-250 siswa',
        226,
        250,
        90,
        74000
    ),
    (
        112,
        8,
        '226-250 siswa',
        226,
        250,
        100,
        81000
    ),
    (
        113,
        8,
        '226-250 siswa',
        226,
        250,
        110,
        88000
    ),
    (
        114,
        8,
        '226-250 siswa',
        226,
        250,
        120,
        92000
    ),
    (
        115,
        8,
        '226-250 siswa',
        226,
        250,
        135,
        96000
    ),
    (
        116,
        8,
        '226-250 siswa',
        226,
        250,
        150,
        100000
    ),
    (
        117,
        8,
        '226-250 siswa',
        226,
        250,
        160,
        104000
    ),
    (
        118,
        9,
        '251-275 siswa',
        251,
        275,
        30,
        41000
    ),
    (
        119,
        9,
        '251-275 siswa',
        251,
        275,
        45,
        49000
    ),
    (
        120,
        9,
        '251-275 siswa',
        251,
        275,
        60,
        55000
    ),
    (
        121,
        9,
        '251-275 siswa',
        251,
        275,
        65,
        58000
    ),
    (
        122,
        9,
        '251-275 siswa',
        251,
        275,
        75,
        64000
    ),
    (
        123,
        9,
        '251-275 siswa',
        251,
        275,
        80,
        64000
    ),
    (
        124,
        9,
        '251-275 siswa',
        251,
        275,
        90,
        70000
    ),
    (
        125,
        9,
        '251-275 siswa',
        251,
        275,
        100,
        76000
    ),
    (
        126,
        9,
        '251-275 siswa',
        251,
        275,
        110,
        83000
    ),
    (
        127,
        9,
        '251-275 siswa',
        251,
        275,
        120,
        86000
    ),
    (
        128,
        9,
        '251-275 siswa',
        251,
        275,
        135,
        90000
    ),
    (
        129,
        9,
        '251-275 siswa',
        251,
        275,
        150,
        94000
    ),
    (
        130,
        9,
        '251-275 siswa',
        251,
        275,
        160,
        98000
    ),
    (
        131,
        10,
        '276-300 siswa',
        276,
        300,
        30,
        39000
    ),
    (
        132,
        10,
        '276-300 siswa',
        276,
        300,
        45,
        46000
    ),
    (
        133,
        10,
        '276-300 siswa',
        276,
        300,
        60,
        52000
    ),
    (
        134,
        10,
        '276-300 siswa',
        276,
        300,
        65,
        55000
    ),
    (
        135,
        10,
        '276-300 siswa',
        276,
        300,
        75,
        60000
    ),
    (
        136,
        10,
        '276-300 siswa',
        276,
        300,
        80,
        60000
    ),
    (
        137,
        10,
        '276-300 siswa',
        276,
        300,
        90,
        66000
    ),
    (
        138,
        10,
        '276-300 siswa',
        276,
        300,
        100,
        72000
    ),
    (
        139,
        10,
        '276-300 siswa',
        276,
        300,
        110,
        78000
    ),
    (
        140,
        10,
        '276-300 siswa',
        276,
        300,
        120,
        81000
    ),
    (
        141,
        10,
        '276-300 siswa',
        276,
        300,
        135,
        85000
    ),
    (
        142,
        10,
        '276-300 siswa',
        276,
        300,
        150,
        89000
    ),
    (
        143,
        10,
        '276-300 siswa',
        276,
        300,
        160,
        92000
    ),
    (
        144,
        11,
        '301-325 siswa',
        301,
        325,
        30,
        37000
    ),
    (
        145,
        11,
        '301-325 siswa',
        301,
        325,
        45,
        44000
    ),
    (
        146,
        11,
        '301-325 siswa',
        301,
        325,
        60,
        49000
    ),
    (
        147,
        11,
        '301-325 siswa',
        301,
        325,
        65,
        52000
    ),
    (
        148,
        11,
        '301-325 siswa',
        301,
        325,
        75,
        57000
    ),
    (
        149,
        11,
        '301-325 siswa',
        301,
        325,
        80,
        57000
    ),
    (
        150,
        11,
        '301-325 siswa',
        301,
        325,
        90,
        62000
    ),
    (
        151,
        11,
        '301-325 siswa',
        301,
        325,
        100,
        68000
    ),
    (
        152,
        11,
        '301-325 siswa',
        301,
        325,
        110,
        74000
    ),
    (
        153,
        11,
        '301-325 siswa',
        301,
        325,
        120,
        77000
    ),
    (
        154,
        11,
        '301-325 siswa',
        301,
        325,
        135,
        80000
    ),
    (
        155,
        11,
        '301-325 siswa',
        301,
        325,
        150,
        84000
    ),
    (
        156,
        11,
        '301-325 siswa',
        301,
        325,
        160,
        87000
    ),
    (
        157,
        12,
        '326-350 siswa',
        326,
        350,
        30,
        35000
    ),
    (
        158,
        12,
        '326-350 siswa',
        326,
        350,
        45,
        42000
    ),
    (
        159,
        12,
        '326-350 siswa',
        326,
        350,
        60,
        47000
    ),
    (
        160,
        12,
        '326-350 siswa',
        326,
        350,
        65,
        50000
    ),
    (
        161,
        12,
        '326-350 siswa',
        326,
        350,
        75,
        54000
    ),
    (
        162,
        12,
        '326-350 siswa',
        326,
        350,
        80,
        54000
    ),
    (
        163,
        12,
        '326-350 siswa',
        326,
        350,
        90,
        59000
    ),
    (
        164,
        12,
        '326-350 siswa',
        326,
        350,
        100,
        65000
    ),
    (
        165,
        12,
        '326-350 siswa',
        326,
        350,
        110,
        70000
    ),
    (
        166,
        12,
        '326-350 siswa',
        326,
        350,
        120,
        73000
    ),
    (
        167,
        12,
        '326-350 siswa',
        326,
        350,
        135,
        76000
    ),
    (
        168,
        12,
        '326-350 siswa',
        326,
        350,
        150,
        80000
    ),
    (
        169,
        12,
        '326-350 siswa',
        326,
        350,
        160,
        83000
    ),
    (
        170,
        13,
        '351-375 siswa',
        351,
        375,
        30,
        33000
    ),
    (
        171,
        13,
        '351-375 siswa',
        351,
        375,
        45,
        40000
    ),
    (
        172,
        13,
        '351-375 siswa',
        351,
        375,
        60,
        45000
    ),
    (
        173,
        13,
        '351-375 siswa',
        351,
        375,
        65,
        47000
    ),
    (
        174,
        13,
        '351-375 siswa',
        351,
        375,
        75,
        52000
    ),
    (
        175,
        13,
        '351-375 siswa',
        351,
        375,
        80,
        52000
    ),
    (
        176,
        13,
        '351-375 siswa',
        351,
        375,
        90,
        56000
    ),
    (
        177,
        13,
        '351-375 siswa',
        351,
        375,
        100,
        62000
    ),
    (
        178,
        13,
        '351-375 siswa',
        351,
        375,
        110,
        67000
    ),
    (
        179,
        13,
        '351-375 siswa',
        351,
        375,
        120,
        70000
    ),
    (
        180,
        13,
        '351-375 siswa',
        351,
        375,
        135,
        73000
    ),
    (
        181,
        13,
        '351-375 siswa',
        351,
        375,
        150,
        76000
    ),
    (
        182,
        13,
        '351-375 siswa',
        351,
        375,
        160,
        79000
    ),
    (
        183,
        14,
        '376-400 siswa',
        376,
        400,
        30,
        31000
    ),
    (
        184,
        14,
        '376-400 siswa',
        376,
        400,
        45,
        38000
    ),
    (
        185,
        14,
        '376-400 siswa',
        376,
        400,
        60,
        42000
    ),
    (
        186,
        14,
        '376-400 siswa',
        376,
        400,
        65,
        45000
    ),
    (
        187,
        14,
        '376-400 siswa',
        376,
        400,
        75,
        49000
    ),
    (
        188,
        14,
        '376-400 siswa',
        376,
        400,
        80,
        49000
    ),
    (
        189,
        14,
        '376-400 siswa',
        376,
        400,
        90,
        53000
    ),
    (
        190,
        14,
        '376-400 siswa',
        376,
        400,
        100,
        58000
    ),
    (
        191,
        14,
        '376-400 siswa',
        376,
        400,
        110,
        63000
    ),
    (
        192,
        14,
        '376-400 siswa',
        376,
        400,
        120,
        66000
    ),
    (
        193,
        14,
        '376-400 siswa',
        376,
        400,
        135,
        69000
    ),
    (
        194,
        14,
        '376-400 siswa',
        376,
        400,
        150,
        72000
    ),
    (
        195,
        14,
        '376-400 siswa',
        376,
        400,
        160,
        75000
    ),
    (
        196,
        15,
        '401-425 siswa',
        401,
        425,
        30,
        30000
    ),
    (
        197,
        15,
        '401-425 siswa',
        401,
        425,
        45,
        36000
    ),
    (
        198,
        15,
        '401-425 siswa',
        401,
        425,
        60,
        40000
    ),
    (
        199,
        15,
        '401-425 siswa',
        401,
        425,
        65,
        42000
    ),
    (
        200,
        15,
        '401-425 siswa',
        401,
        425,
        75,
        46000
    ),
    (
        201,
        15,
        '401-425 siswa',
        401,
        425,
        80,
        46000
    ),
    (
        202,
        15,
        '401-425 siswa',
        401,
        425,
        90,
        50000
    ),
    (
        203,
        15,
        '401-425 siswa',
        401,
        425,
        100,
        55000
    ),
    (
        204,
        15,
        '401-425 siswa',
        401,
        425,
        110,
        60000
    ),
    (
        205,
        15,
        '401-425 siswa',
        401,
        425,
        120,
        62000
    ),
    (
        206,
        15,
        '401-425 siswa',
        401,
        425,
        135,
        65000
    ),
    (
        207,
        15,
        '401-425 siswa',
        401,
        425,
        150,
        68000
    ),
    (
        208,
        15,
        '401-425 siswa',
        401,
        425,
        160,
        71000
    ),
    (
        209,
        16,
        '426-450 siswa',
        426,
        450,
        30,
        28000
    ),
    (
        210,
        16,
        '426-450 siswa',
        426,
        450,
        45,
        34000
    ),
    (
        211,
        16,
        '426-450 siswa',
        426,
        450,
        60,
        38000
    ),
    (
        212,
        16,
        '426-450 siswa',
        426,
        450,
        65,
        40000
    ),
    (
        213,
        16,
        '426-450 siswa',
        426,
        450,
        75,
        44000
    ),
    (
        214,
        16,
        '426-450 siswa',
        426,
        450,
        80,
        44000
    ),
    (
        215,
        16,
        '426-450 siswa',
        426,
        450,
        90,
        48000
    ),
    (
        216,
        16,
        '426-450 siswa',
        426,
        450,
        100,
        52000
    ),
    (
        217,
        16,
        '426-450 siswa',
        426,
        450,
        110,
        57000
    ),
    (
        218,
        16,
        '426-450 siswa',
        426,
        450,
        120,
        59000
    ),
    (
        219,
        16,
        '426-450 siswa',
        426,
        450,
        135,
        62000
    ),
    (
        220,
        16,
        '426-450 siswa',
        426,
        450,
        150,
        65000
    ),
    (
        221,
        16,
        '426-450 siswa',
        426,
        450,
        160,
        67000
    ),
    (
        222,
        17,
        '451-475 siswa',
        451,
        475,
        30,
        27000
    ),
    (
        223,
        17,
        '451-475 siswa',
        451,
        475,
        45,
        32000
    ),
    (
        224,
        17,
        '451-475 siswa',
        451,
        475,
        60,
        36000
    ),
    (
        225,
        17,
        '451-475 siswa',
        451,
        475,
        65,
        38000
    ),
    (
        226,
        17,
        '451-475 siswa',
        451,
        475,
        75,
        42000
    ),
    (
        227,
        17,
        '451-475 siswa',
        451,
        475,
        80,
        42000
    ),
    (
        228,
        17,
        '451-475 siswa',
        451,
        475,
        90,
        45000
    ),
    (
        229,
        17,
        '451-475 siswa',
        451,
        475,
        100,
        50000
    ),
    (
        230,
        17,
        '451-475 siswa',
        451,
        475,
        110,
        54000
    ),
    (
        231,
        17,
        '451-475 siswa',
        451,
        475,
        120,
        56000
    ),
    (
        232,
        17,
        '451-475 siswa',
        451,
        475,
        135,
        59000
    ),
    (
        233,
        17,
        '451-475 siswa',
        451,
        475,
        150,
        62000
    ),
    (
        234,
        17,
        '451-475 siswa',
        451,
        475,
        160,
        64000
    ),
    (
        235,
        18,
        '476-500 siswa',
        476,
        500,
        30,
        26000
    ),
    (
        236,
        18,
        '476-500 siswa',
        476,
        500,
        45,
        31000
    ),
    (
        237,
        18,
        '476-500 siswa',
        476,
        500,
        60,
        34000
    ),
    (
        238,
        18,
        '476-500 siswa',
        476,
        500,
        65,
        36000
    ),
    (
        239,
        18,
        '476-500 siswa',
        476,
        500,
        75,
        40000
    ),
    (
        240,
        18,
        '476-500 siswa',
        476,
        500,
        80,
        40000
    ),
    (
        241,
        18,
        '476-500 siswa',
        476,
        500,
        90,
        43000
    ),
    (
        242,
        18,
        '476-500 siswa',
        476,
        500,
        100,
        47000
    ),
    (
        243,
        18,
        '476-500 siswa',
        476,
        500,
        110,
        51000
    ),
    (
        244,
        18,
        '476-500 siswa',
        476,
        500,
        120,
        53000
    ),
    (
        245,
        18,
        '476-500 siswa',
        476,
        500,
        135,
        56000
    ),
    (
        246,
        18,
        '476-500 siswa',
        476,
        500,
        150,
        59000
    ),
    (
        247,
        18,
        '476-500 siswa',
        476,
        500,
        160,
        61000
    );

-- --------------------------------------------------------

--
-- Table structure for table `tbl_fs_prices`
--

CREATE TABLE `tbl_fs_prices` (
    `id` int NOT NULL,
    `pkg` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `min_siswa` int DEFAULT NULL,
    `max_siswa` int DEFAULT NULL,
    `harga` int DEFAULT NULL,
    `pages` int DEFAULT NULL
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `tbl_fs_prices`
--

INSERT INTO
    `tbl_fs_prices` (
        `id`,
        `pkg`,
        `min_siswa`,
        `max_siswa`,
        `harga`,
        `pages`
    )
VALUES (
        1,
        'handy',
        30,
        50,
        465000,
        30
    ),
    (
        2,
        'handy',
        51,
        75,
        415000,
        30
    ),
    (
        3,
        'handy',
        76,
        100,
        370000,
        45
    ),
    (
        4,
        'handy',
        101,
        125,
        350000,
        55
    ),
    (
        5,
        'handy',
        126,
        150,
        335000,
        60
    ),
    (
        6,
        'handy',
        151,
        175,
        315000,
        65
    ),
    (
        7,
        'handy',
        176,
        200,
        295000,
        75
    ),
    (
        8,
        'handy',
        201,
        225,
        260000,
        80
    ),
    (
        9,
        'handy',
        226,
        250,
        250000,
        80
    ),
    (
        10,
        'handy',
        251,
        275,
        240000,
        90
    ),
    (
        11,
        'handy',
        276,
        300,
        230000,
        100
    ),
    (
        12,
        'handy',
        300,
        325,
        220000,
        100
    ),
    (
        13,
        'handy',
        326,
        350,
        210000,
        120
    ),
    (
        14,
        'handy',
        351,
        375,
        200000,
        120
    ),
    (
        15,
        'handy',
        376,
        400,
        190000,
        135
    ),
    (
        16,
        'handy',
        401,
        425,
        185000,
        135
    ),
    (
        17,
        'handy',
        426,
        450,
        165000,
        145
    ),
    (
        18,
        'handy',
        451,
        475,
        175000,
        150
    ),
    (
        19,
        'handy',
        476,
        500,
        150000,
        160
    ),
    (
        20,
        'minimal',
        30,
        50,
        450000,
        30
    ),
    (
        21,
        'minimal',
        51,
        75,
        400000,
        30
    ),
    (
        22,
        'minimal',
        76,
        100,
        355000,
        45
    ),
    (
        23,
        'minimal',
        101,
        125,
        335000,
        55
    ),
    (
        24,
        'minimal',
        126,
        150,
        320000,
        60
    ),
    (
        25,
        'minimal',
        151,
        175,
        300000,
        65
    ),
    (
        26,
        'minimal',
        176,
        200,
        280000,
        75
    ),
    (
        27,
        'minimal',
        201,
        225,
        245000,
        80
    ),
    (
        28,
        'minimal',
        226,
        250,
        235000,
        80
    ),
    (
        29,
        'minimal',
        251,
        275,
        240000,
        90
    ),
    (
        30,
        'minimal',
        276,
        300,
        215000,
        100
    ),
    (
        31,
        'minimal',
        300,
        325,
        205000,
        100
    ),
    (
        32,
        'minimal',
        326,
        350,
        195000,
        120
    ),
    (
        33,
        'minimal',
        351,
        375,
        185000,
        120
    ),
    (
        34,
        'minimal',
        376,
        400,
        180000,
        135
    ),
    (
        35,
        'minimal',
        401,
        425,
        170000,
        135
    ),
    (
        36,
        'minimal',
        426,
        450,
        160000,
        145
    ),
    (
        37,
        'minimal',
        451,
        475,
        150000,
        150
    ),
    (
        38,
        'minimal',
        476,
        500,
        140000,
        160
    ),
    (
        39,
        'large',
        30,
        50,
        480000,
        30
    ),
    (
        40,
        'large',
        51,
        75,
        430000,
        30
    ),
    (
        41,
        'large',
        76,
        100,
        405000,
        45
    ),
    (
        42,
        'large',
        101,
        125,
        365000,
        55
    ),
    (
        43,
        'large',
        126,
        150,
        350000,
        60
    ),
    (
        44,
        'large',
        151,
        175,
        330000,
        65
    ),
    (
        45,
        'large',
        176,
        200,
        310000,
        75
    ),
    (
        46,
        'large',
        201,
        225,
        275000,
        80
    ),
    (
        47,
        'large',
        226,
        250,
        265000,
        80
    ),
    (
        48,
        'large',
        251,
        275,
        255000,
        90
    ),
    (
        49,
        'large',
        276,
        300,
        245000,
        100
    ),
    (
        50,
        'large',
        300,
        325,
        235000,
        100
    ),
    (
        51,
        'large',
        326,
        350,
        225000,
        120
    ),
    (
        52,
        'large',
        351,
        375,
        215000,
        120
    ),
    (
        53,
        'large',
        376,
        400,
        205000,
        135
    ),
    (
        54,
        'large',
        401,
        425,
        195000,
        135
    ),
    (
        55,
        'large',
        426,
        450,
        175000,
        145
    ),
    (
        56,
        'large',
        451,
        475,
        165000,
        150
    ),
    (
        57,
        'large',
        476,
        500,
        155000,
        160
    );

-- --------------------------------------------------------

--
-- Table structure for table `tbl_grad_addons`
--

CREATE TABLE `tbl_grad_addons` (
    `id` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `category` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `name` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `price` int DEFAULT NULL
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `tbl_grad_addons`
--

INSERT INTO
    `tbl_grad_addons` (
        `id`,
        `category`,
        `name`,
        `price`
    )
VALUES (
        'g10r',
        'cetak',
        'Cetak Foto 10R',
        15000
    ),
    (
        'g12r',
        'cetak',
        'Cetak Foto 12R',
        20000
    ),
    (
        'g4r',
        'cetak',
        'Cetak Foto 4R',
        4000
    ),
    (
        'g8r',
        'cetak',
        'Cetak Foto 8R',
        8000
    ),
    (
        'gbooth_add',
        'addons',
        'Tambah 1 Jam Photobooth/360',
        500000
    ),
    (
        'gphoto_add',
        'addons',
        'Tambah 1 Fotografer',
        1250000
    ),
    (
        'gvideo_add',
        'addons',
        'Tambah 1 Videografer',
        1500000
    ),
    (
        'gwork_add',
        'addons',
        'Tambah 1 Jam Kerja/Orang',
        350000
    );

-- --------------------------------------------------------

--
-- Table structure for table `tbl_grad_packages`
--

CREATE TABLE `tbl_grad_packages` (
    `id` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `name` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `price` int DEFAULT NULL,
    `description` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    `color` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `tbl_grad_packages`
--

INSERT INTO
    `tbl_grad_packages` (
        `id`,
        `name`,
        `price`,
        `description`,
        `color`
    )
VALUES (
        'g360',
        'Glamation 360',
        4100000,
        '1-2 Crew profesional',
        ''
    ),
    (
        'gbooth',
        'Photo Booth',
        3850000,
        '1-2 Crew profesional',
        ''
    ),
    (
        'gcomplete',
        'Complete Package',
        7750000,
        'Photo + Video',
        'feat'
    ),
    (
        'gphoto',
        'Photo Only',
        2750000,
        '2 Fotografer',
        ''
    ),
    (
        'gphv',
        'Photo & Video',
        4500000,
        '2 Fotografer + 1 Videografer',
        'acc'
    ),
    (
        'gvideo',
        'Video Only',
        2000000,
        '1 Videografer',
        ''
    );

-- --------------------------------------------------------

--
-- Table structure for table `tbl_multipliers`
--

CREATE TABLE `tbl_multipliers` (
    `id` int NOT NULL,
    `category` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `key_name` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `label` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `value` float DEFAULT NULL
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `tbl_multipliers`
--

INSERT INTO
    `tbl_multipliers` (
        `id`,
        `category`,
        `key_name`,
        `label`,
        `value`
    )
VALUES (
        1,
        'cetak',
        'handy',
        'Handy Book A4+',
        1
    ),
    (
        2,
        'cetak',
        'minimal',
        'Minimal Book SQ',
        0.95
    ),
    (
        3,
        'cetak',
        'large',
        'Large Book B4',
        1.15
    ),
    (
        4,
        'alc',
        'ebook',
        'E-Book Package (%)',
        72
    ),
    (
        5,
        'alc',
        'editcetak',
        'Edit+Desain+Cetak (%)',
        62
    ),
    (
        6,
        'alc',
        'desain',
        'Desain Only (%)',
        22
    ),
    (
        7,
        'alc',
        'cetakonly',
        'Cetak Only (%)',
        30
    );

-- --------------------------------------------------------

--
-- Table structure for table `tbl_overhead`
--

CREATE TABLE `tbl_overhead` (
    `id` int NOT NULL,
    `key_name` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `label` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `amount` int DEFAULT NULL
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `tbl_overhead`
--

INSERT INTO
    `tbl_overhead` (
        `id`,
        `key_name`,
        `label`,
        `amount`
    )
VALUES (
        1,
        'total',
        'Total Overhead',
        65540000
    ),
    (
        2,
        'marketing',
        'Marketing & Pemasaran',
        12750000
    ),
    (
        3,
        'creative',
        'Tim Kreatif (Desain, Video)',
        7670000
    ),
    (
        4,
        'designer',
        'Fotografer & Tim Lapangan',
        16700000
    ),
    (
        5,
        'pm',
        'Project Manager',
        7200000
    ),
    (
        6,
        'sosmed',
        'Sosmed & Konten',
        6430000
    ),
    (
        7,
        'freelance',
        'Cadangan Freelance',
        3204000
    ),
    (
        8,
        'ops',
        'Ops (Tools, Transport, Server)',
        11586000
    );

-- --------------------------------------------------------

--
-- Table structure for table `users`
--

CREATE TABLE `users` (
    `id` int NOT NULL,
    `username` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `password` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `name` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
    `position` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `role_id` int NOT NULL,
    `is_active` tinyint(1) DEFAULT '1',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

--
-- Dumping data for table `users`
--

INSERT INTO
    `users` (
        `id`,
        `username`,
        `password`,
        `name`,
        `position`,
        `role_id`,
        `is_active`,
        `created_at`
    )
VALUES (
        1,
        'admin',
        '$2y$10$JVcYQCJ9bOxiKKzjD5AaIuFm0JsI.wTCpCsh4WjjMlTELw7aRE0J2',
        'Administrator',
        NULL,
        1,
        1,
        '2026-04-05 17:00:00'
    ),
    (
        2,
        'manager',
        '$2y$12$6vCDcax64AynAFSzqXszoOuIG71InJs.36/xgr/IMvosizHtRry7q',
        'Dabo',
        'Digital Marketing',
        2,
        1,
        '2026-04-05 17:00:00'
    ),
    (
        3,
        'staff',
        '$2y$10$xwONKMp6sripzA5QZRFBTuoYPhXqIdTvi/Tt0VU5LneOeJjMu6cse',
        'Staff Member',
        NULL,
        3,
        1,
        '2026-04-05 17:00:00'
    ),
    (
        4,
        'Bimo',
        '$2y$12$AOlPLpJE.4kTutEDVzto5uyrSOPnncVUUohiNCx8mcaaGns6fC1qO',
        'Bimo',
        NULL,
        3,
        1,
        '2026-04-06 03:08:38'
    ),
    (
        5,
        'Hani',
        '$2y$12$FEQay1jLDud4s1ha52aYEOIEaTDPdMuoUZmyMR9g9yFw6uIe5U5Mu',
        'Hani',
        NULL,
        3,
        1,
        '2026-04-17 02:26:36'
    ),
    (
        6,
        'Cahyostaff',
        '$2y$12$I9g3zd9Jo4CjihFtaJq/SuXKWH0Jl3xNxcaVJjYBIEPxYk.xx7DdS',
        'cahyo adi nugroho',
        'Manager',
        1,
        1,
        '2026-06-04 09:21:25'
    );

--
-- Indexes for dumped tables
--

--
-- Indexes for table `addons`
--
ALTER TABLE `addons`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `name` (`name`),
ADD KEY `idx_active` (`active`),
ADD KEY `idx_category` (`category`);

--
-- Indexes for table `addon_categories`
--
ALTER TABLE `addon_categories`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `category_name` (`category_name`);

--
-- Indexes for table `addon_items`
--
ALTER TABLE `addon_items`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `name_category` (`name`, `category_id`),
ADD KEY `idx_addon_cat` (`category_id`);

--
-- Indexes for table `addon_tiers`
--
ALTER TABLE `addon_tiers`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `tier_range` (
    `addon_item_id`,
    `min_quantity`
),
ADD KEY `idx_addon_tier` (`addon_item_id`);

--
-- Indexes for table `alacarte_config`
--
ALTER TABLE `alacarte_config`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `package_code` (`package_code`),
ADD KEY `idx_active` (`active`),
ADD KEY `idx_order` (`display_order`);

--
-- Indexes for table `alacarte_factors`
--
ALTER TABLE `alacarte_factors`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `package_code` (`package_code`);

--
-- Indexes for table `bonus_fasilitas`
--
ALTER TABLE `bonus_fasilitas`
ADD PRIMARY KEY (`id`),
ADD KEY `idx_pkg_active` (
    `package_type`,
    `active`,
    `display_order`
);

--
-- Indexes for table `cetak_base`
--
ALTER TABLE `cetak_base`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `range_pages` (`min_students`, `pages_count`),
ADD KEY `idx_cetak_range` (`min_students`, `pages_count`);

--
-- Indexes for table `cetak_factors`
--
ALTER TABLE `cetak_factors`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `package_type` (`package_type`);

--
-- Indexes for table `fullservice_pricing`
--
ALTER TABLE `fullservice_pricing`
ADD PRIMARY KEY (`id`),
ADD KEY `idx_package` (`package_type`),
ADD KEY `idx_students` (
    `min_students`,
    `max_students`
),
ADD KEY `idx_active` (`active`);

--
-- Indexes for table `graduation_addons`
--
ALTER TABLE `graduation_addons`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `addon_key` (`addon_key`);

--
-- Indexes for table `graduation_cetak`
--
ALTER TABLE `graduation_cetak`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `cetak_key` (`cetak_key`),
ADD KEY `idx_active` (`active`);

--
-- Indexes for table `jasa_termasuk`
--
ALTER TABLE `jasa_termasuk` ADD PRIMARY KEY (`id`);

--
-- Indexes for table `overhead`
--
ALTER TABLE `overhead`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `category` (`category`);

--
-- Indexes for table `overhead_total`
--
ALTER TABLE `overhead_total` ADD PRIMARY KEY (`id`);

--
-- Indexes for table `packages_alacarte`
--
ALTER TABLE `packages_alacarte`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `code` (`code`);

--
-- Indexes for table `packages_config`
--
ALTER TABLE `packages_config`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `package_code` (`package_code`),
ADD KEY `idx_active` (`active`),
ADD KEY `idx_order` (`display_order`);

--
-- Indexes for table `packages_fullservice`
--
ALTER TABLE `packages_fullservice`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `type_range` (
    `package_type`,
    `min_students`
),
ADD KEY `idx_fs_type` (`package_type`);

--
-- Indexes for table `packages_graduation`
--
ALTER TABLE `packages_graduation`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `package_key` (`package_key`),
ADD KEY `idx_grad_pkg` (`package_key`);

--
-- Indexes for table `payment_terms`
--
ALTER TABLE `payment_terms`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `term_name` (`term_name`),
ADD KEY `idx_active` (`active`);

--
-- Indexes for table `pdf_ketentuan`
--
ALTER TABLE `pdf_ketentuan`
ADD PRIMARY KEY (`id`),
ADD KEY `idx_pkg` (`package_type`);

--
-- Indexes for table `penawaran`
--
ALTER TABLE `penawaran`
ADD PRIMARY KEY (`id`),
ADD KEY `added_by` (`added_by`);

--
-- Indexes for table `pricing_factors`
--
ALTER TABLE `pricing_factors`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `unique_factor` (`category`, `factor_name`),
ADD KEY `idx_category` (`category`);

--
-- Indexes for table `roles`
--
ALTER TABLE `roles`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `name` (`name`);

--
-- Indexes for table `settings`
--
ALTER TABLE `settings`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `setting_key` (`setting_key`);

--
-- Indexes for table `spesifikasi_produk`
--
ALTER TABLE `spesifikasi_produk` ADD PRIMARY KEY (`id`);

--
-- Indexes for table `syarat_ketentuan`
--
ALTER TABLE `syarat_ketentuan` ADD PRIMARY KEY (`id`);

--
-- Indexes for table `tbl_addons`
--
ALTER TABLE `tbl_addons` ADD PRIMARY KEY (`id`);

--
-- Indexes for table `tbl_cetak_prices`
--
ALTER TABLE `tbl_cetak_prices` ADD PRIMARY KEY (`id`);

--
-- Indexes for table `tbl_fs_prices`
--
ALTER TABLE `tbl_fs_prices` ADD PRIMARY KEY (`id`);

--
-- Indexes for table `tbl_grad_addons`
--
ALTER TABLE `tbl_grad_addons` ADD PRIMARY KEY (`id`);

--
-- Indexes for table `tbl_grad_packages`
--
ALTER TABLE `tbl_grad_packages` ADD PRIMARY KEY (`id`);

--
-- Indexes for table `tbl_multipliers`
--
ALTER TABLE `tbl_multipliers`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `key_name` (`key_name`);

--
-- Indexes for table `tbl_overhead`
--
ALTER TABLE `tbl_overhead`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `key_name` (`key_name`);

--
-- Indexes for table `users`
--
ALTER TABLE `users`
ADD PRIMARY KEY (`id`),
ADD UNIQUE KEY `username` (`username`),
ADD KEY `role_id` (`role_id`);

--
-- AUTO_INCREMENT for dumped tables
--

--
-- AUTO_INCREMENT for table `addons`
--
ALTER TABLE `addons`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 58;

--
-- AUTO_INCREMENT for table `addon_categories`
--
ALTER TABLE `addon_categories`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 7;

--
-- AUTO_INCREMENT for table `addon_items`
--
ALTER TABLE `addon_items`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 10;

--
-- AUTO_INCREMENT for table `addon_tiers`
--
ALTER TABLE `addon_tiers`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 29;

--
-- AUTO_INCREMENT for table `alacarte_config`
--
ALTER TABLE `alacarte_config`
MODIFY `id` int UNSIGNED NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 5;

--
-- AUTO_INCREMENT for table `alacarte_factors`
--
ALTER TABLE `alacarte_factors`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 5;

--
-- AUTO_INCREMENT for table `bonus_fasilitas`
--
ALTER TABLE `bonus_fasilitas`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 14;

--
-- AUTO_INCREMENT for table `cetak_base`
--
ALTER TABLE `cetak_base`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 95127;

--
-- AUTO_INCREMENT for table `cetak_factors`
--
ALTER TABLE `cetak_factors`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 4;

--
-- AUTO_INCREMENT for table `fullservice_pricing`
--
ALTER TABLE `fullservice_pricing`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 172;

--
-- AUTO_INCREMENT for table `graduation_addons`
--
ALTER TABLE `graduation_addons`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 44;

--
-- AUTO_INCREMENT for table `graduation_cetak`
--
ALTER TABLE `graduation_cetak`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 33;

--
-- AUTO_INCREMENT for table `jasa_termasuk`
--
ALTER TABLE `jasa_termasuk`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 21;

--
-- AUTO_INCREMENT for table `overhead`
--
ALTER TABLE `overhead`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 361;

--
-- AUTO_INCREMENT for table `overhead_total`
--
ALTER TABLE `overhead_total`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 3;

--
-- AUTO_INCREMENT for table `packages_alacarte`
--
ALTER TABLE `packages_alacarte`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 10;

--
-- AUTO_INCREMENT for table `packages_config`
--
ALTER TABLE `packages_config`
MODIFY `id` int UNSIGNED NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 12;

--
-- AUTO_INCREMENT for table `packages_fullservice`
--
ALTER TABLE `packages_fullservice`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 13396;

--
-- AUTO_INCREMENT for table `packages_graduation`
--
ALTER TABLE `packages_graduation`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 61;

--
-- AUTO_INCREMENT for table `payment_terms`
--
ALTER TABLE `payment_terms`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 21;

--
-- AUTO_INCREMENT for table `pdf_ketentuan`
--
ALTER TABLE `pdf_ketentuan`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 7;

--
-- AUTO_INCREMENT for table `penawaran`
--
ALTER TABLE `penawaran`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 14;

--
-- AUTO_INCREMENT for table `pricing_factors`
--
ALTER TABLE `pricing_factors`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 22;

--
-- AUTO_INCREMENT for table `roles`
--
ALTER TABLE `roles`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 4;

--
-- AUTO_INCREMENT for table `settings`
--
ALTER TABLE `settings`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 11;

--
-- AUTO_INCREMENT for table `spesifikasi_produk`
--
ALTER TABLE `spesifikasi_produk`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 16;

--
-- AUTO_INCREMENT for table `syarat_ketentuan`
--
ALTER TABLE `syarat_ketentuan`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 12;

--
-- AUTO_INCREMENT for table `tbl_addons`
--
ALTER TABLE `tbl_addons`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 3777;

--
-- AUTO_INCREMENT for table `tbl_cetak_prices`
--
ALTER TABLE `tbl_cetak_prices`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 248;

--
-- AUTO_INCREMENT for table `tbl_fs_prices`
--
ALTER TABLE `tbl_fs_prices`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 58;

--
-- AUTO_INCREMENT for table `tbl_multipliers`
--
ALTER TABLE `tbl_multipliers`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 8;

--
-- AUTO_INCREMENT for table `tbl_overhead`
--
ALTER TABLE `tbl_overhead`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 9;

--
-- AUTO_INCREMENT for table `users`
--
ALTER TABLE `users`
MODIFY `id` int NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 7;

--
-- Constraints for dumped tables
--

--
-- Constraints for table `addon_items`
--
ALTER TABLE `addon_items`
ADD CONSTRAINT `addon_items_ibfk_1` FOREIGN KEY (`category_id`) REFERENCES `addon_categories` (`id`) ON DELETE CASCADE;

--
-- Constraints for table `addon_tiers`
--
ALTER TABLE `addon_tiers`
ADD CONSTRAINT `addon_tiers_ibfk_1` FOREIGN KEY (`addon_item_id`) REFERENCES `addon_items` (`id`) ON DELETE CASCADE;

--
-- Constraints for table `penawaran`
--
ALTER TABLE `penawaran`
ADD CONSTRAINT `penawaran_ibfk_1` FOREIGN KEY (`added_by`) REFERENCES `users` (`id`) ON DELETE SET NULL;

--
-- Constraints for table `users`
--
ALTER TABLE `users`
ADD CONSTRAINT `users_ibfk_1` FOREIGN KEY (`role_id`) REFERENCES `roles` (`id`);

COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */
;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */
;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */
;