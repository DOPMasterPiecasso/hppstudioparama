<?php
/**
 * Parama HPP — Jasa Termasuk API
 *
 * CRUD master data jasa termasuk per tipe paket.
 * Data ini menggantikan hardcode di pdf.php.
 *
 * GET    /api/jasa-termasuk.php                     — list semua (grouped by package_type)
 * GET    /api/jasa-termasuk.php?type=fullservice     — list per tipe
 * POST   /api/jasa-termasuk.php  {action:'create', label, package_type, kategori, display_order?}
 * POST   /api/jasa-termasuk.php  {action:'update', id, label?, package_type?, kategori?, display_order?, active?}
 * POST   /api/jasa-termasuk.php  {action:'delete', id}
 */
    ...
    if ($method === 'POST') {
        ...
        switch ($action) {
            ...
            case 'create': {
                ...
                $stmt = $pdo->prepare(
                    "INSERT INTO jasa_termasuk (package_type, kategori, label, detail, display_order)
                     VALUES (?, ?, ?, ?, ?)"
                );
                ...
                echo json_encode([
                    'success' => true,
                    'message' => 'Jasa Termasuk berhasil ditambahkan.',
                    'id'      => $newId,
            ...
            case 'update': {
                ...
                $params[] = $id;
                $sql = 'UPDATE jasa_termasuk SET ' . implode(', ', $fields) . ' WHERE id = ?';
                $pdo->prepare($sql)->execute($params);

                echo json_encode(['success' => true, 'message' => 'Jasa Termasuk berhasil diupdate.'], JSON_UNESCAPED_UNICODE);
                break;
            }
            ...
            case 'delete': {
                ...
                $pdo->prepare("DELETE FROM jasa_termasuk WHERE id = ?")->execute([$id]);
                echo json_encode(['success' => true, 'message' => 'Jasa Termasuk dihapus.'], JSON_UNESCAPED_UNICODE);
                break;
            }
            ...
            case 'reorder': {
                ...
                $stmt = $pdo->prepare("UPDATE jasa_termasuk SET display_order = ? WHERE id = ?");
